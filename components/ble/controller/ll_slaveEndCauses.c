 ...
 * Functions
 */
uint32 ll_adptive_adj_next_time(uint32_t nextTime);
...
//    sw_delay = (T2 > ISR_entry_time) ? (T2 - ISR_entry_time) : (BASE_TIME_UNITS - ISR_entry_time + T2);      
    ...
    next_time = ll_adptive_adj_next_time(next_time);      
...
{
	uint32 adj_time = 0;
	uint32 next_time = nextTime;
	uint32   T2 = 0;
    uint32_t anchor_point;//,irq_status;
	...
	if(connPtr->rx_timeout)
		slave_conn_event_recv_delay += ((connPtr->ll_buf.ntrm_cnt) * pGlobal_config[LL_WRITE_RE_TX_FIFO]);
	
	T2 = read_current_fine_time();
	
	if( connPtr->rx_timeout )
	{
		// g_TIM1_IRQ_timing : last Timer1 irq entry pending(delay) time
		adj_time = slave_conn_event_recv_delay + LL_TIME_DELTA(ISR_entry_time,T2 ) + g_TIM1_IRQ_timing + pGlobal_config[TIMER_ISR_ENTRY_TIME];
	}
	else
	{
		adj_time = slave_conn_event_recv_delay + LL_TIME_DELTA(ISR_entry_time,T2 ) + connPtr->timerDrift +	\
					pGlobal_config[LL_HW_BB_DELAY] + pGlobal_config[NEXT_TIMER1_CONSIDER_DELAY] + pGlobal_config[TIMER_ISR_ENTRY_TIME];
	}
