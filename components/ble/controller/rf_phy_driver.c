...
{
    uint8_t len = 0;
    uint32_t currTick,deltTick;
    uint32_t pllMoniter;
    //---------------------------------------------------------------------------------------- 
    //get input param
    //
    uint8_t lastDtmModType = g_dtmModeType;
    
    g_dtmModeType   =  cmdWord        &  0xff;
    uint8_t rfChn   = (cmdWord>> 8)   &  0xff;
    g_dtmLength     = ((cmdWord>>16)  &  0x0f)<<3;      
    g_dtmPKT        = (cmdWord>>20)   &  0x0f;
    uint8_t loopN   = (cmdWord>>24)   &  0xff;        // for ll trig times

    //g_dtmLength = ((0x03&g_dtmExtLen)<<6)|(0x3f&g_dtmLength);
    int pktLenUs =0;                        //(g_dtmLength+10)<<3;

    uint8_t preambleLen = 7;
    ...
    if(g_dtmModeType == RF_PHY_DTM_MODE_ATE_RESET)
    {
        ...
        len                 =   1;
        dOut[0]             =   g_dtmModeType;

    }
    else if(g_dtmModeType == RF_PHY_DTM_MODE_ATE_SET_TXPOWER)
    {
        
        g_dtmTxPower        =   (cmdWord>>8) & 0x1f;
        len                 =   1;
        dOut[0]             =   g_dtmModeType;
        
    }
    else if(g_dtmModeType == RF_PHY_DTM_MODE_ATE_SET_PKTFMT)
    {
    ...
    else if(   g_dtmModeType == RF_PHY_DTM_MODE_ATE_TX_BURST
             || g_dtmModeType == RF_PHY_DTM_MODE_ATE_TX_MOD
             || g_dtmModeType == RF_PHY_DTM_MODE_ATE_TX_CARR   )
    {
        ...
        if(g_dtmModeType == RF_PHY_DTM_MODE_ATE_TX_CARR)
        {
        ...
        if(g_dtmModeType == RF_PHY_DTM_MODE_ATE_TX_BURST)
        { 
            //[15:8] payload Len [7:5] preamble len, [4] tx mode, [3:0] payload type     
            PHY_REG_WT(0x40030040,(0x00030010|(g_dtmLength<<8)|(preambleLen<<5)|(0x0f & g_dtmPKT)) );
        }
        ...
        {
            //[15:8] payload Len [7:5] preamble len, [4] tx mode, [3:0] payload type     
            PHY_REG_WT(0x40030040,0x00030010|(preambleLen<<5)|(0x0f & g_dtmPKT));

            rf_phy_set_txPower(0x1f&(cmdWord>>24));      // for CTMOD and CARRIER set txpower
            
        }
        
        //Fix TX_TEST_MOD ZIGBEE ISSUE
        if(g_rfPhyPktFmt==0)
        {
            PHY_REG_WT(0x40030000,0x36068002);//use BLE2M without Gaussfilter
            PHY_REG_WT(0x40030040,0x00080000 | (PHY_REG_RD(0x40030040)));
        }
...
//                     PHY_REG_WT( 0x400300a4,0x00000154);     // clr tx_auto
        ...
        else if(g_dtmModeType == RF_PHY_DTM_MODE_ATE_TX_CARR)
        {
    ...
    else if(   g_dtmModeType == RF_PHY_DTM_MODE_ATE_RX_AUTOGAIN 
             || g_dtmModeType == RF_PHY_DTM_MODE_ATE_RX_FIXGAIN
             || g_dtmModeType == RF_PHY_DTM_MODE_ATE_RX_DEMOD
             || g_dtmModeType == RF_PHY_DTM_MODE_ATE_RX2TX
             || g_dtmModeType == RF_PHY_DTM_MODE_ATE_TX2RX)
   {
        ...
        if(     g_dtmModeType == RF_PHY_DTM_MODE_ATE_RX2TX)
        {
            ll_hw_set_rtx();
        ...
        else if(g_dtmModeType == RF_PHY_DTM_MODE_ATE_TX2RX)
        {         
        ...
        if(     g_dtmModeType == RF_PHY_DTM_MODE_ATE_RX_AUTOGAIN 
            ||  g_dtmModeType == RF_PHY_DTM_MODE_ATE_RX_FIXGAIN )
        {
        ...
        else
        {
            ...
            while(loopN)
            {
                ...
                if(ll_hw_get_irq_status()&LIRQ_MD)
                {
                    ...
                    if(ll_hw_get_irq_status()&LIRQ_COK)
                    {
                     ...
                     if(g_dtmModeType == RF_PHY_DTM_MODE_ATE_TX2RX)
                     {
                         WaitUs(70);
                     }                     
            ...
            len     = 7;
            dOut[0] = g_dtmPktCount;
            dOut[1] = g_dtmRxCrcNum;
            dOut[2] = g_dtmRxTONum;
            dOut[3] = (g_dtmFoff>>8) & 0xff;
            dOut[4] = (g_dtmFoff     & 0xff);
            dOut[5] = g_dtmRssi;
            dOut[6] = g_dtmCarrSens;
        }//end of !RX_ON
