...
void osalTimerUpdate( uint32 updateTime )
{
  ...
  if ( timerHead != NULL )
  {
    ...
    while ( srchTimer )
    {
      ...
      if (0) // arm cpu, so use 32bit math  //(timeUnion.time16[1] == 0) && (timeUnion.time8[1] == 0))
      {
      ...
      // Check for reloading
      if ( (srchTimer->timeout.time32 == 0) &&
           (srchTimer->reloadTimeout) && (srchTimer->event_flag) )     // case 1. trigger event & reload
      {
      ...
      if ( (srchTimer->timeout.time32 == 0) ||                 // update by HZF, use 32bit compare
      ...
      if ( freeTimer )                         // case 2. trigger event & free
      {
        if (freeTimer->timeout.time32 == 0) // (freeTimer->timeout.time16[0] == 0) && (freeTimer->timeout.time16[1] == 0) )
        {
