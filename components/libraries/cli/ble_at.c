...
uint8 bleAT_TaskID;
extern void connHdlMapRoleInit(void);
...
uint16_t bleAT_ProcessEvent( uint8 task_id, uint16_t events )
{
    ...
    if ( events & AT_START_DEVICE_EVT )
    {
        ...
        connHdlMapRoleInit();
        ///set sleep handler
        hal_pwrmgr_register(MOD_USR1, at_sleep_handle, at_wakeup_handle);
    ...
    if ( events & AT_PROCESS_UART_RX_CMD_EVT )
    {
        ...
        {
            if((cmdstr[0] == 'A') && (cmdstr[1] == 'T') && (cmdstr[2] == '+')) /// wake up flag "AT+"
            {
                cmdstr[cmdlen] = '\0';
            ...
            else
            {
                ...
                {
                    osal_start_timerEx( bleAT_TaskID, AT_ENTER_ATUO_SLEEP_MODE_EVT,  5*10 );
                }
            }
