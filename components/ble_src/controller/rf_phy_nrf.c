...
extern uint8_t  phyBufRx[256];
extern uint8_t  phyBufTx[256];
...
uint8_t nrf_pkt_enc(nrfPkt_t* p_pkt,uint8_t* dOut, uint8_t* doLen)
{
    ...
    if(nrfConfig.mode == NRF_MODE_ENHANCE_SHOCKBURST)
    {
        ...
        dOut[bof] = (crc[0]<<1) | (dOut[bof]   &0x01);
        bof++;
        dOut[bof] = (crc[1]<<1) | ((crc[0]>>7) &0x01);
        bof++;

        //fill the last bit
        if(nrfConfig.crcByte==CRC_ITU_16_LEN_2BYTE)
            dOut[bof] = (0        ) | ((crc[1]>>7) &0x01);
