...
volatile uint16_t        g_dtmManualConfig   =   RF_PHY_DTM_MANUL_ALL;
...
void rf_phy_ana_cfg(void)
{
    ...
    //-------------------------------------------------------------------
    //TX PLL BW
    PHY_REG_WT(0x400300d0,0x00000160);  // i_pll_ctrl1
...
void rf_phy_bb_cfg(uint8_t pktFmt)
{
    ...
    if(pktFmt==0 || pktFmt==2)
    {
        ...
        if(g_rfPhyClkSel == RF_PHY_CLK_SEL_16M_XTAL)
            ...
            PHY_REG_WT( 0x40030090,0x00044500);   // set reg_dc
    ...
    else if(pktFmt<5)
    {
        ...
        if(g_rfPhyClkSel == RF_PHY_CLK_SEL_16M_XTAL)
            ...
            PHY_REG_WT( 0x40030090,0x00022000);   // set reg_dc
    ...
    PHY_REG_WT( 0x400300b0,0x01000002);                 // dac dly
...
void rf_phy_direct_test(void)
{
    ...
    if(g_dtmManualConfig & RF_PHY_DTM_MANUL_FOFF)
        DTM_LOG("=== DTM MANUL FOFF  ===\n");

    if(g_dtmManualConfig & RF_PHY_DTM_MANUL_TXPOWER)
        DTM_LOG("=== DTM MANUL TXPOWER  ===\n");

    if(g_dtmManualConfig & RF_PHY_DTM_MANUL_XTAL_CAP)
        DTM_LOG("=== DTM MANUL XTAL_CAP  ===\n");

    if(g_dtmManualConfig & RF_PHY_DTM_CONFIG_MAX_GAIN)
        DTM_LOG("=== DTM CONFIG MAX GAIN  ===\n");

    if(g_dtmManualConfig & RF_PHY_DTM_CONFIG_RX_STABLE_MOD)
        DTM_LOG("=== DTM CONFIG RX STABLE MOD  ===\n");
...
//                            }
                           ...
                           ||  g_dtmModeType == RF_PHY_DTM_MODE_GET_PER_AUTO)
                {
                    ...
                    if(g_dtmManualConfig&RF_PHY_DTM_CONFIG_RX_STABLE_MOD)
                    {
                        PHY_REG_WT(0x40030008,0x04600175);     //config ahdfe para 1dB better
                        // best sens wo dirty Tx
                        // test under dirty Tx should turn off this config
                    }
                    else
                    {
                        PHY_REG_WT(0x40030008,0x0c680192);     //config ahdfe para 0.4dB better sens
                    }
