...
/*********************************************************************
    API FUNCTIONS
*/
uint8 smRegisterPairingContent(uint8 max_conn_num, smPairingParams_t** param)
{
    if(( param == NULL ) || \
            ((max_conn_num == 0) || ( max_conn_num > MAX_NUM_LL_CONN_ROM_LIMT ) ))
    {
    ...
    LOG("%s\n",__func__);
    LOG("pPairingParams %p\n",pPairingParams);

    for(uint8 i=0; i<max_conn_num; i++)
        LOG("pPairingParams[i] %p\n",&pPairingParams[i]);
                           ...
                           smLinkSecurityReq_t* pSecReqs )
{
    ...
    LOG("%s, pPairingParams[connectionHandle] %p\n",__func__, pPairingParams[connectionHandle]);
    ...
    LOG("mem alloc, pPairingParams[connectionHandle] %p\n",pPairingParams[connectionHandle]);
...
void smProcessDataMsg( l2capDataEvent_t* pMsg )
{
    ...
    LOG("smpProcessIncoming failed.reason 0x%X\n",failed.reason);
...
{
    LOG("%s\n",__func__);
    LOG("pPairingParams[%d] = %p , status %d\n",connectionHandle,pPairingParams[connectionHandle],status);
