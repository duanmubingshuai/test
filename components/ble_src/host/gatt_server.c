...
gattServerInfo_t* serverInfoTbl;
uint8 gattServerNum = 0;
...
/*********************************************************************
    API FUNCTIONS
*/
uint8 gattServer_stackInitContent(uint8 max_gattServerNum,gattServerInfo_t* serverInfo)
{
    if( serverInfo != NULL)
    {
        gattServerNum = max_gattServerNum;
        serverInfoTbl = serverInfo;
        LOG("%s,gattServerNum %d,serverInfoTbl %p\n\n\n",__func__,gattServerNum,serverInfoTbl);
        return SUCCESS;
    }
...
bStatus_t GATT_InitServer( void )
{
    ...
    for ( uint8 i = 0; i < gattServerNum; i++ )
    {
