...
static uint8 smpResponderProcessIncoming( linkDBItem_t* pLinkItem, uint8 cmdID, smpMsgs_t* pParsedMsg )
{
    ...
    LOG("%s,connHandle %d,pPairingParams[%d]=%p \n",__func__,pLinkItem->connectionHandle,pLinkItem->connectionHandle,pPairingParams[pLinkItem->connectionHandle]);
    ...
    if ( pPairingParams[pLinkItem->connectionHandle] == NULL )
    {
        if ( cmdID == SMP_PAIRING_REQ )
        {
            ...
            // Notify the app
            LOG("cmdID == SMP_PAIRING_REQ  Notify the app\n");
