...
gapAuthStateParams_t** pAuthLink;
...
/*********************************************************************
    API FUNCTIONS
*/
uint8 gap_AuthenLink_InitContent(uint8 max_conn_num,gapAuthStateParams_t** Authen_link)
{
    if( Authen_link == NULL )
    ...
    {
//      pAuthLink[i] = Authen_link[i];
        LOG("%s,Authen_link %p,pAuthLink[%d] %p\n",__func__,Authen_link,i,&pAuthLink[i]);
    }
...
bStatus_t GAP_Authenticate( gapAuthParams_t* pParams, gapPairingReq_t* pPairReq )
{
    ...
    LOG("pAuthLink[%d]\n",pParams->connectionHandle,pAuthLink[pParams->connectionHandle]);
...
/*********************************************************************
    @fn          gapProcessEnhConnectionCompleteEvt

    @brief       Process the LL HCI enhenced Connection Complete Event
    ...
    @return      none
*/
void gapProcessEnhConnectionCompleteEvt( hciEvt_BLEEnhConnComplete_t* pPkt )
{
    uint8 appTaskID;
