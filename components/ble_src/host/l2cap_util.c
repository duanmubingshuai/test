...
/*********************************************************************
    GLOBAL VARIABLES
*/
l2capReassemblePkt_t* l2capReassemblePkt;
l2capSegmentBuff_t*   l2capSegmentPkt;
l2capSARDbugCnt_t g_sarDbgCnt;
uint8 l2capExtendFragments = TRUE;
/*********************************************************************
    EXTERNAL VARIABLES
*/
extern uint8 linkDB_Max_num;
...
uint8 l2cap_stack_InitContent(uint8 max_conn_num,l2capReassemblePkt_t* ressembleBuf,l2capSegmentBuff_t* segmentBuf)
{
    LOG("%s,linkDB_Max_num %d max_conn_num %d\n",__func__,linkDB_Max_num,max_conn_num);
    LOG("ressembleBuf %p,segmentBuf %p\n",ressembleBuf,segmentBuf);

    if( ((max_conn_num == 0) || ( max_conn_num > MAX_NUM_LL_CONN_ROM_LIMT ) ) || \
            (ressembleBuf == NULL ) || (segmentBuf == NULL))
        return INVALIDPARAMETER;

    l2capReassemblePkt = ressembleBuf;
    l2capSegmentPkt = segmentBuf;
//  osal_memset(l2capReassemblePkt , 0 ,sizeof(l2capReassemblePkt_t) * max_conn_num );
//  osal_memset(l2capSegmentPkt , 0 ,sizeof(l2capSegmentBuff_t) * max_conn_num );
    LOG("l2capReassemblePkt %p,l2capSegmentPkt %p\n",l2capReassemblePkt,l2capSegmentPkt);
    return SUCCESS;
}
...
//                                                         pPkt->pPayload ,len,CID);
            ...
            else
            {
                ...
                if(( len + L2CAP_HDR_SIZE ) < pHciMsg->len) ///bugfix: hciLen > l2cap len  2022 11 04
                {
                    g_sarDbgCnt.reassembleErrCID++;
                    return ( TRUE );
                }
