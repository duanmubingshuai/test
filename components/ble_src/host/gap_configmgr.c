...
/*ExtAdv Set static Address*/
bStatus_t gapExtSetStaticAddr( uint8 advhandle,uint8* pNewAddr )
{
    gapDeviceAddrMode = ADDRTYPE_STATIC;
    VOID osal_memcpy( gapCurrentRandomAddr, pNewAddr, B_ADDR_LEN );
    VOID gapAddAddrAdj( gapDeviceAddrMode, gapCurrentRandomAddr );
    HCI_LE_SetExtAdvSetRandomAddressCmd(advhandle,gapCurrentRandomAddr);
    return SUCCESS;
}
