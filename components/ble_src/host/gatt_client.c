...
gattClientInfo_t* clientInfoTbl;
uint8 gattClientNum = 0;
...
/*********************************************************************
    API FUNCTIONS
*/
uint8 gattClient_stackInitContent(uint8 max_gattClientNum,gattClientInfo_t* clientInfo)
{
    if( clientInfo != NULL)
    {
        gattClientNum = max_gattClientNum;
        clientInfoTbl = clientInfo;
        LOG("%s,gattClientNum %d,clientInfoTbl %p\n",__func__,gattClientNum,clientInfoTbl);
        return SUCCESS;
    }
...
static bStatus_t gattReadByGrpType( uint16 connHandle, attReadByGrpTypeReq_t* pReq, uint8 taskId )
{
    ...
    if ( status == SUCCESS )
    {
        ...
        LOG("ATT_ReadByGrpTypeReq,status %d\n",status);
...
static bStatus_t gattClientProcessMsgCB( uint16 connHandle, attPacket_t* pPkt )
{
    ...
    if ( ( pPkt->method == ATT_HANDLE_VALUE_NOTI ) || ( pPkt->method == ATT_HANDLE_VALUE_IND ) )
    {
        ...
        if ( status == SUCCESS )
        {
            if(pPkt->method == ATT_HANDLE_VALUE_IND)
            {
                ATT_HandleValueCfm(connHandle);
            }
