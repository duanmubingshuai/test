...
{
	if(pev->data[0] == 'i' || pev->data[0] == 3)
	{
		osal_set_event(pwmlight_TaskID, PWMLIGHT_EVT_FADE_IN);
	}
	else if(pev->data[0] == 'o' || pev->data[0] == 4)
	{
		osal_set_event(pwmlight_TaskID, PWMLIGHT_EVT_FADE_OUT);
	}
	else if(pev->data[0] == 'r' || pev->data[0] == 0)
	{
		osal_set_event(pwmlight_TaskID, PWMLIGHT_EVT_R_ONOFF);
	}
	else if(pev->data[0] == 'g' || pev->data[0] == 1)
	{
		osal_set_event(pwmlight_TaskID, PWMLIGHT_EVT_G_ONOFF);
	}
	else if(pev->data[0] == 'b' || pev->data[0] == 2)
	{
		osal_set_event(pwmlight_TaskID, PWMLIGHT_EVT_B_ONOFF);
	}
	else if(pev->data[0] == 'a' || pev->data[0] == 5)
	{
		osal_set_event(pwmlight_TaskID, PWMLIGHT_EVT_ONOFF);
	}
	else if(pev->data[0] == 'h' || pev->data[0] == 6)
	{
		osal_set_event(pwmlight_TaskID, PWMLIGHT_EVT_HALF);
	}
}


static bool key_debounce = false;
void key_hdl(gpio_pin_e pin,gpio_polarity_e type)
{
  if(key_debounce == false)
    osal_set_event(pwmlight_TaskID, PWMLIGHT_EVT_ONOFF);
  key_debounce = true;
  osal_start_timerEx(pwmlight_TaskID, PWMLIGHT_EVT_200MS, 200);
  
}


void pwmlight_cmd(uint16_t cmd_evt)
{
  osal_set_event(pwmlight_TaskID, cmd_evt);
}
...
void pwmlight_onoff_all(bool onoff)
{
  ...
  {
    g_pwm_chcfg[0].cmp_val = 0;
    g_pwm_chcfg[1].cmp_val = 0;
    g_pwm_chcfg[2].cmp_val = 0;
...
void pwmlight_Init(uint8 task_id)
{
  ...
  gpio_pull_set(LIGHT_R, GPIO_PULL_UP);
  gpio_pull_set(LIGHT_G, GPIO_PULL_UP);
  gpio_pull_set(LIGHT_B, GPIO_PULL_UP);
  ...
  gpio_pull_set(P0, GPIO_PULL_UP);
  gpioin_register(P0, key_hdl, NULL);
  //LOG("g :%d\n", ret);
  pwm_init();
...
void  pwmlight_phy_control(uint8_t cmd)
{
     uint8_t data[1];
     data[0] = cmd;
    comm_evt_t temp;
