...
//  if ( ((gapProfileRole & GAP_PROFILE_BROADCASTER))
...
//  osal_stop_timerEx( gapTaskID, GAP_END_ADVERTISING_EVT );
 ...
 *          off to another task.
 */
uint8 gapPeriProcessHCICmdCompleteEvt( hciEvt_CmdComplete_t *pMsg )
{
  uint8 safeToDealloc = TRUE;
	#ifdef _PHY_DEBUG 
		  LOG("%s,%s,Line %d,pMsg->cmdOpcode 0x%X\n",__FILE__,__func__,__LINE__,pMsg->cmdOpcode);
	#endif
...
//          osal_start_reload_timer( gapTaskID, GAP_END_ADVERTISING_EVT,
...
//      if ( timeout > 0 )
//      {
//        osal_start_timerEx( gapTaskID, GAP_END_ADVERTISING_EVT, timeout );
//      }
...
//    GAP_EndDiscoverable( pGapAdvertState->taskID );
//  }
//}
 ...
 * @return      none
 */
void gapConnectedCleanUpAdvertising( void )
{
  gapFreeAdvertState();
}
...
static void gapSendMakeDiscEvent( bStatus_t status, uint16 interval )
{
  ...
  if ( pGapAdvertState )
  {
    ...
    if ( pRsp )
    {
      ...
      osal_msg_send( pGapAdvertState->taskID, (uint8 *)pRsp );
    }
    if ( status != SUCCESS )
    {
...
static void gapSendAdDataUpdateEvent( uint8 adType, uint8 status )
{
  if ( gapAdvAppTaskID != INVALID_TASK_ID )
  {
    ...
    if ( pRsp )
    {
      ...
      osal_msg_send( gapAdvAppTaskID, (uint8 *)pRsp );
    }
  }
}
