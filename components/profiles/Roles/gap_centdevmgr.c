...
void gapCentProcessHCICmdEvt( uint16 cmdOpcode, hciEvt_CmdComplete_t *pMsg );
...
//  }
  ...
  if ( pGapDiscReq )
  {
    ...
    // Copy parameters
    osal_memcpy( pGapDiscReq, pParams, (unsigned int)(sizeof (  gapDevDiscReq_t )) );
...
//  osal_stop_timerEx( gapTaskID, GAP_OSAL_TIMER_SCAN_DURATION_EVT );
...
void gapCentProcessHCICmdEvt( uint16 cmdOpcode, hciEvt_CmdComplete_t *pMsg )
{
  ...
  switch ( cmdOpcode )
  {
    case HCI_LE_SET_SCAN_ENABLE:
      ...
      {
      	#ifdef _PHY_DEBUG 
			LOG("HCI_LE_SET_SCAN_ENABLE success \n");
		#endif
        break;
      }
      /*lint --fallthrough */
    case HCI_LE_SET_SCAN_PARAM:
      gapSetScanParamStatus( *(pMsg->pReturnParam) );
      break;
  }
}
...
//    osal_msg_send( pGapDiscReq->taskID, (uint8 *)pMsg );
//  }
//
