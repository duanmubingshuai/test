...
API_RESULT netif_send (NETIF_PKT_T type, MS_NET_ADDR d_addr, UCHAR* pdata, UINT16 pdatalen,UINT8 tx_flag)
{
    API_RESULT retval;
    UINT32     index;
    MS_BUFFER  buffer;
...
//            "[netIF 0] Sent PDU over ADV Bearer. Result 0x%04X\n", retval);
    ...
    {
        if ((BRR_HANDLE_INVALID != netif_brr_handle[index])&& tx_flag)
            //(!unsegment_flag || (unsegment_flag && tx_flag)) )
        {
            BRR_TYPE b_type;
            b_type = ((NETIF_PKT_T_PROXY == type) /*|| (NETIF_PKT_T_RELAY == type)*/) ? BRR_SUBTYPE_GATT_PROXY_T_MASK : BRR_SUBTYPE_GATT_NETWORK_T_MASK;
            NET_TRC(
