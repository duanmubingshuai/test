...
/* Proxy beacon pointer */
DECL_STATIC PROXY_BCON_CB proxy_bcon_callback;
...
void proxy_handle_nodeid_beacon(UCHAR* pdata, UINT16 pdatalen)
{
...
/* Secure Network Beacon Handler */
void proxy_handle_networkid_beacon(UCHAR* pdata, UINT16 pdatalen)
{
    PROXY_BCON_STATE_PARAMS state_params;
    UINT8 marker = 0;
...
API_RESULT net_proxy_init(void)
{
    ...
    for (index = 0; index < (UINT32)PROXY_FILTER_LIST_COUNT; index ++)
    {
        ...
        MS_INIT_GLOBAL_ARRAY(PROXY_ADDR, net_proxy_list[index].v_addr, MS_CONFIG_LIMITS(MS_PROXY_FILTER_DYNAMIC_LIST_SIZE), 0x00);
        net_proxy_list[index].v_count = 0;

        for (i = 0; i < ms_proxy_filter_list_size; i++)
        {
            if(net_proxy_list[index].p_addr[i] != MS_NET_ADDR_UNASSIGNED)
            {
                net_proxy_list[index].p_count++;
            }
        }
...
void net_proxy_iface_down(NETIF_HANDLE* handle)
{
    ...
    for (index = 0; index < (UINT32)PROXY_FILTER_LIST_COUNT; index ++)
    {
        ...
        (
            ...
            0x0,
            (sizeof(PROXY_ADDR) * MS_CONFIG_LIMITS(MS_PROXY_FILTER_DYNAMIC_LIST_SIZE))
        );
...
)
{
    ...
    EM_mem_set
    (
        net_proxy_list[(*handle - 1)].p_addr,
        0x0,
        (sizeof(PROXY_ADDR) * ms_proxy_filter_list_size)
    );
    net_proxy_list[(*handle - 1)].p_count = 0;
    /* Set the incoming filter type for the SubNet's Proxy List */
...
)
{
    ...
    /*  Check if the PDU len received is greater than equal to 2 and is a
        multiple of 2
    */
    if ((2 > pdu_len) || (0x0001 & pdu_len) ||
            ((opcode==MS_PROXY_ADD_TO_FILTER_OPCODE)&&
             (((proxy_fitlter_flg == MS_FALSE) &&(net_proxy_list[(*handle - 1)].p_count >= ms_proxy_filter_list_size))||
              ((proxy_fitlter_flg == MS_TRUE) &&(net_proxy_list[(*handle - 1)].v_count >= MS_PROXY_FILTER_DYNAMIC_LIST_SIZE)))))
    {
        NET_ERR(
    ...
    case MS_PROXY_ADD_TO_FILTER_OPCODE:
        ...
        if(proxy_fitlter_flg == MS_TRUE)
        {
            for (i1 = 0; i1 < t_count; i1++)
            {
                ...
                for (i2 = 0; i2 < ms_proxy_filter_list_size; i2++)
                {
//                  NET_TRC (
...
//                      "[PROXY]:i2 0x%x, 0x%04X ,0x%04x ,0x%04x\n",i2,net_proxy_list[(*handle - 1)].p_addr[i2], t_addr, net_proxy_list[(*handle - 1)].v_addr[i2]);
                ...
                if ((ms_proxy_filter_list_size == i2) && (MS_CONFIG_LIMITS(MS_PROXY_FILTER_DYNAMIC_LIST_SIZE) == i3) &&
                        (MS_CONFIG_LIMITS(MS_PROXY_FILTER_DYNAMIC_LIST_SIZE) > net_proxy_list[(*handle - 1)].v_count))
                {
                    /* Index to be populated */
                    i3 = net_proxy_list[(*handle - 1)].v_count;
                    net_proxy_list[(*handle - 1)].v_addr[i3] = t_addr;
                    ...
                    for (i = 0; i < MS_CONFIG_LIMITS(MS_PROXY_FILTER_DYNAMIC_LIST_SIZE); i++)
                    {
                        NET_TRC (
...
//          proxy_first_fitlter = MS_FALSE;
        ...
        else
        {
            for (i1 = 0; i1 < t_count; i1++)
            {
                ...
                /* Not present in the filter list and list is not full */
                if ((ms_proxy_filter_list_size == i2) &&
                        (ms_proxy_filter_list_size > net_proxy_list[(*handle - 1)].p_count))
                {
                    /* Index to be populated */
        ...
        for (i1 = 0; i1 < t_count; i1++)
        {
            ...
            for (i2 = 0; i2 < ms_proxy_filter_list_size; i2++)
            {
                if (net_proxy_list[(*handle - 1)].p_addr[i2] == t_addr)
                {
                    ...
                    {
                        add = net_proxy_list[(*handle - 1)].p_addr[i2];
                        net_proxy_list[(*handle - 1)].p_addr[i2] = net_proxy_list[(*handle - 1)].p_addr[net_proxy_list[(*handle - 1)].p_count - 1];
                        net_proxy_list[(*handle - 1)].p_addr[net_proxy_list[(*handle - 1)].p_count - 1] = add;
                        net_proxy_list[(*handle - 1)].p_addr[net_proxy_list[(*handle - 1)].p_count -1] = MS_NET_ADDR_UNASSIGNED;
                        net_proxy_list[(*handle - 1)].p_count--;
                    }
                    else
                    {
...
)
{
    ...
    if((d_addr == MS_NET_ADDR_ALL_NODES)||(pkt_sub_type == BRR_SUBTYPE_GATT_PROXY_T_MASK))
        return API_SUCCESS;
    ...
    #endif /* MS_PROXY_CLIENT */
    {
        ...
        */
        ...
        {
            /* Check if Address is present in Subnet's proxy list */
            for (i = 0; i < ms_proxy_filter_list_size + MS_CONFIG_LIMITS(MS_PROXY_FILTER_DYNAMIC_LIST_SIZE); i++)
            {
                if(((i < ms_proxy_filter_list_size)&&(net_proxy_list[(*handle - 1)].p_addr[i] == d_addr)) ||
                        (((i >= ms_proxy_filter_list_size)&&(net_proxy_list[(*handle - 1)].v_addr[i - ms_proxy_filter_list_size] == d_addr))))
                {
                    break;
                }
            }
            ...
            {
                retval = (i < ms_proxy_filter_list_size + MS_CONFIG_LIMITS(MS_PROXY_FILTER_DYNAMIC_LIST_SIZE)) ? API_SUCCESS : API_FAILURE;
                NET_TRC (
            ...
            else
            {
                ...
                */
                retval = (i < ms_proxy_filter_list_size + MS_CONFIG_LIMITS(MS_PROXY_FILTER_DYNAMIC_LIST_SIZE)) ? API_FAILURE : API_SUCCESS;
                NET_TRC (
...
)
{
    /* LOCK */
    /* Register the callback */
    proxy_bcon_callback = proxy_bcon_cb;
    /* UNLOCK */
    return API_SUCCESS;
}
...
API_RESULT MS_proxy_bcon_handler_register(void)
{
    ...
    if(retval == API_SUCCESS)
    {
        ...
        (
            MS_BCON_TYPE_PROXY_NETID,
            proxy_handle_networkid_beacon
        );
    }
...
)
{
    ...
    else if (MS_PROXY_PRIV_ADV_MODE == proxy_adv_mode)
    {
        ...
        if (EM_TIMER_HANDLE_INIT_VAL != net_proxy_priv_timer_handle)
        {
            NET_ERR (
                "[PROXY]: Proxy Adv on priv already in progress\n");
            retval = EM_stop_timer(&net_proxy_priv_timer_handle);
        }

        MS_brr_bcast_end(BRR_BCON_TYPE_PROXY_PRIV, BRR_BCON_ACTIVE);
        MS_brr_start_priv_proxy_adv
        (
...
//      retval = EM_start_timer
//      (
//          &net_proxy_priv_timer_handle,
//          PROXY_SUBNET_NETID_ADV_TIMEOUT,
//          net_proxy_priv_timeout_handler,
//          &handle,
//          sizeof(handle)
//      );
//
...
API_RESULT MS_private_server_adv_stop (void)
{
    ...
    retval = API_SUCCESS;
    NET_TRC (
        "[PROXY]: >> MS_private_server_adv_stop\n");

    /**
        The Proxy ADV Mode here is Don't Care.
        Hence, using default value of mode as MS_PROXY_NET_ID_ADV_MODE.
    */
    if (MS_PROXY_PRIV_ADV_MODE & private_adv_state)
    {
        MS_brr_bcast_end(BRR_BCON_TYPE_PROXY_PRIV, BRR_BCON_ACTIVE);
        private_adv_state &= (UCHAR)(~BRR_BCON_TYPE_PROXY_PRIV);
    }
    ...
    */
    NET_TRC (
        "[PROXY]: << MS_private_server_adv_stop, retval 0x%04X\n", retval);
    return retval;
}

void net_proxy_priv_timeout_handler(void* args, UINT16 size)
{
    UCHAR  net_id[26];
    API_RESULT retval;
    UINT16 handle;
    ...
    if (!(private_adv_state & MS_PROXY_PRIV_ADV_MODE))
    {
        return;
    }
