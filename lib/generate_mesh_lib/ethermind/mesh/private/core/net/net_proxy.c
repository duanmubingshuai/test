...
API_RESULT net_proxy_init(void)
{
    ...
    #ifdef MS_PROXY_SERVER
    MS_INIT_GLOBAL_ARRAY(PROXY_FILTER_LIST, net_proxy_list, PROXY_FILTER_LIST_COUNT, 0x00);
...
void net_proxy_iface_down(NETIF_HANDLE* handle)
{
    ...
    for (index = 0; index < (UINT32)PROXY_FILTER_LIST_COUNT; index ++)
    {
        ...
        net_proxy_list[(*handle - 1)].count = 0;
    ...
    case MS_PROXY_ADD_TO_FILTER_OPCODE:
        for (i1 = 0; i1 < t_count; i1++)
        {
            ...
            if ((MS_CONFIG_LIMITS(MS_PROXY_FILTER_LIST_SIZE) == i2) &&
                    (MS_CONFIG_LIMITS(MS_PROXY_FILTER_LIST_SIZE) != net_proxy_list[(*handle - 1)].count))
            {
                /* Index to be populated */
                i2 = net_proxy_list[(*handle - 1)].count;
                net_proxy_list[(*handle - 1)].p_addr[i2] = t_addr;
                net_proxy_list[(*handle - 1)].count++;
                NET_TRC (
                    "[PROXY]: Added 0x%04X to List of SubNet 0x%02X\n", t_addr, *handle);
            }
            else
            {
                NET_ERR(
        ...
        for (i1 = 0; i1 < t_count; i1++)
        {
            ...
            for (i2 = 0; i2 < MS_CONFIG_LIMITS(MS_PROXY_FILTER_LIST_SIZE); i2++)
            {
                if (net_proxy_list[(*handle - 1)].p_addr[i2] == t_addr)
                {
                    ...
                    if (0 != net_proxy_list[(*handle - 1)].count)
                    {
                        net_proxy_list[(*handle - 1)].count--;
                    }
                    else
...
)
{
    ...
    MS_PACK_BE_2_BYTE_VAL(&pdu[2], net_proxy_list[(*handle - 1)].count);
    net_proxy_send
    (
        subnet_handle,
        pdu,
    ...
    if (BRR_SERVER_ROLE == net_proxy_list[(*handle - 1)].role)
    {
        ...
        else
        {
            NET_ERR (
                ...
                " for handle 0x%02X!!\n",
                net_proxy_list[(*handle - 1)].count, *handle);
        }
    }
    else
    {
    ...
    #endif /* MS_PROXY_CLIENT */
    {
        ...
        */
        ...
        if (MS_NET_ADDR_UNASSIGNED != d_addr)
        {
            ...
            {
                if (net_proxy_list[(*handle - 1)].p_addr[i] == d_addr)
                {
                    break;
                }
            }
