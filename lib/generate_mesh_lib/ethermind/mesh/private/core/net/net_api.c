...
#define MS_IV_UPDATE_TIMEOUT        (96*60*60)//(96*3)
//#define MS_IV_UPDATE_TIMEOUT        8*60

#define MS_IV_UPDATE_STEP           (8*60*60)//(8*3)
...
//    hdr->saddr, hdr->daddr, hdr->ivi, hdr->seq_num, hdr->ctl, hdr->ttl, buffer->length, is_relay);
    ...
    */
    ...
    else
    {
        ...
        if (is_relay == NETIF_PKT_T_RELAY)
        {
            type     = NETIF_PKT_T_RELAY;
...
//            }
            ...
            if(is_relay != NETIF_PKT_T_PROXY)
                tx->tx_flag = MS_TRUE;

            tx->tx_count = (UCHAR)((tx_state & 0x07) + 1);

            if((is_seg == MS_FALSE) && (tx->tx_count<3) && (tx->type == NETIF_PKT_T_NETWORK))
            {
                tx->tx_count = 3;
            }
...
//            printf("[NET Tx] Failed to send Tx packet. Result: 0x%04X\r\n", retval);
        ...
        if(tx->tx_interval*10 <= (NET_TX_TIMEOUT&0xFFFF) )
            ...
            netTxTimeOut = (((10*(tx->tx_interval))+(rtc_get_counter()&0x0f)) | EM_TIMEOUT_MILLISEC);
...
static void ms_iv_update_state_change(void)
{
            ...
            ((ms_iv_index.iv_update_state & MS_IV_UPDATE_STATE) == MS_IV_UPDATE_STATE_IN_PROC))
    {
        ...
        else
        {
            ...
            ((ms_iv_index.iv_update_state & MS_IV_UPDATE_STATE) == MS_IV_UPDATE_STATE_IN_PROC))
    {
        //remove timeout,not recieve done beacon,update to normal mode,iv index--
        //remote active->normal,seq number init
        iv_update_flag =
...
API_RESULT MS_net_start_iv_update_timer(UINT8 timer_flag,UINT8 reset_en)
{
    ...
    if(reset_en == MS_FALSE)
    {
        ...
        {
            ms_iv_index.iv_expire_time = MS_IV_UPDATE_LOCAL_COUNT ;
        }
        else
        {
            ms_iv_index.iv_expire_time = ((timer_flag & MS_IV_UPDATE_STATE)
                                          == MS_IV_UPDATE_STATE_IN_PROC) ?
                                         MS_IV_UPDATE_REMOTE_PRO_COUNT :
                                         MS_IV_UPDATE_REMOTE_DONE_COUNT ;
        }
    }
...
//  printf("receive iv %x %x,current iv %x %x\n",iv_index,iv_update_flag,ms_iv_index.iv_index,ms_iv_index.iv_update_state);
    if((iv_index<ms_iv_index.iv_index) || (iv_index >(ms_iv_index.iv_index + 42)) ||
            ((!(/*((ms_iv_index.iv_update_state & MS_IV_UPDATE_ROLE_LOCAL) == MS_IV_UPDATE_ROLE_REMOTE)&&*/
             ...
             (iv_index==ms_iv_index.iv_index)))//case3: active + proc
    {
//      if((ms_iv_index.iv_expire_time != 0)){
//          //receive iv exception case
//          if (EM_TIMER_HANDLE_INIT_VAL == ms_iv_update_timer_handle)
//          {
//              retval = EM_start_timer
//                       (
...
//  printf("receive continue\n");
    ...
    if((should_start_timer == MS_TRUE)&&
            (should_init_seq_number == MS_FALSE))
    {
        //current n,active receive n+1,active / current n,normal receive n+2,active IV recovery
        //current n,normal receive n+1,active IV update
        if(iv_index == (ms_iv_index.iv_index + 1) &&((ms_iv_index.iv_update_state & MS_IV_UPDATE_ACTIVE) != MS_IV_UPDATE_ACTIVE))
        {
//          printf("IV Update\n");
...
//    printf("iv index:0x%02X\n",ivindex);
    ...
    //local iv update exception case
//  if(((iv_phase & MS_IV_UPDATE_ACTIVE) == MS_IV_UPDATE_ACTIVE) && (ms_iv_index.iv_expire_time != 0))
//  {
//      if (EM_TIMER_HANDLE_INIT_VAL == ms_iv_update_timer_handle)
//      {
//          retval = EM_start_timer
...
//  printf("Allocating SeqNum:0x%08X, max seqnum:0x%08x\n", net_seq_number_state.seq_num,net_seq_number_state.block_seq_num_max);
