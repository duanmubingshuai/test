...
void ms_access_init (void)
{
    ...
    MS_INIT_GLOBAL_ARRAY(MS_ACCESS_ELEMENT_TYPE, ms_access_element_list, ms_access_element_count, 0x00);
    MS_INIT_GLOBAL_ARRAY(MS_ACCESS_MODEL_TYPE, ms_access_model_list, ms_access_model_count, 0x00);
...
API_RESULT ms_access_allocate_element(/* OUT */ MS_ACCESS_ELEMENT_HANDLE*    handle)
{
    ...
    for (index = 0; index < ms_access_element_count; index++)
    {
    ...
    if (ms_access_element_count == index)
    {
...
)
{
    ...
    saved_index = ms_access_model_count;
    ...
    */
    ...
    for (index = 0; index < ms_access_model_count; index++)
    {
        ...
        {
            if (ms_access_model_count == saved_index)
            {
                saved_index = index;
            }
        }
...
//    opcode, subnet_handle, appkey_handle);
    ...
    for (model_index = 0; model_index < ms_access_model_count; model_index++)
    {
        /**
            Check
