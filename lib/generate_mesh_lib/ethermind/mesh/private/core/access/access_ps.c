...
//  }
    ...
    memcpy(&element_buf[offset], (void*)&ms_proxy_filter_addr[0], size);
...
void ms_access_ps_store (/* IN */ UINT32 records)
{
    ...
    if(ms_ps_store_disable_flag == MS_FALSE)
    {
        ...
        count = ++ms_ps_count;
        nvsto_write_header_ps(access_nvsto_handle, (((UINT32)ms_crc16_code)<<16)|(count<<8)|0x1); //writing ok
        /* Close the storage */
...
API_RESULT MS_access_ps_crc_check(void)
{
    ...
    nvsto_read_crc16_ps(access_nvsto_handle,&crc16_code_local,sizeof(crc16_code_local));

    if(crc16_code_local == 0xffff)
    {
        /* Close the storage */
        nvsto_close_ps (access_nvsto_handle);
        return API_SUCCESS;
