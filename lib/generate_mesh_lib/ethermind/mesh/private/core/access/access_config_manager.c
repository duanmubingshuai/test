...
API_RESULT ms_access_cm_init(void)
{
    ...
    MS_INIT_GLOBAL_ARRAY(MS_NETKEY_ENTRY, ms_subnet_table, (ms_max_subnets + ms_max_lpns), 0x00);
    ms_access_cm_init_subnet_table();
    /* Initialize AppKey Table */
    MS_INIT_GLOBAL_ARRAY(MS_APPKEY_ENTRY, ms_appkey_table, ms_max_apps, 0x00);
    ms_access_cm_init_appkey_table();
    /* Initialize Device Key Table */
    MS_INIT_GLOBAL_ARRAY(MS_DEV_KEY_ENTRY, ms_dev_key_table, ms_max_dev_keys, 0x00);
    ms_access_cm_init_dev_key_table();
    /* Initialize Element Address Table */
    MS_INIT_GLOBAL_ARRAY(MS_ELEMENT_ADDR_ENTRY, ms_element_addr_table, 1 + ms_max_lpns, 0x00);
    /* Initialize Non-Virtual Address Table */
    MS_INIT_GLOBAL_ARRAY(MS_NON_VIRTUAL_ADDR_ENTRY, ms_non_virtual_addr_table, ms_max_non_virtual_addrs, 0x00);
    /* Initialize Virtual Address Table */
    MS_INIT_GLOBAL_ARRAY(MS_VIRTUAL_ADDR_ENTRY, ms_virtual_addr_table, ms_max_virtual_addrs, 0x00);
...
void ms_access_cm_init_subnet_table(void)
{
    ...
    for (index = 0; index < (UINT32)(ms_max_subnets + ms_max_lpns); index ++)
    {
...
void ms_access_cm_init_dev_key_table(void)
{
    ...
    for (index = 0; index < ms_max_dev_keys; index ++)
    {
...
)
{
    ...
    if (MS_TRUE == MS_IS_UNICAST_ADDR(addr))
    {
        ...
        for (index = 0; index < (UINT32)(1 + ms_max_lpns); index ++)
        {
            /**
                 ACCESS_TRC(
...
)
{
    ...
    if (ms_dev_key_table_entries >= ms_max_dev_keys)
    {
        ACCESS_ERR(
...
)
{
    ...
    if ((ms_max_dev_keys <= dev_key_index) || (NULL == dev_key))
    {
...
)
{
    ...
    add_index = ms_max_subnets;
    retval = MS_INSUFFICIENT_RESOURCES;
    match_found = MS_FALSE;

    /* Search if the NetKey Index is already present */
    for (index = 0; index < ms_max_subnets; index ++)
    {
        if ((MS_IS_SUBNET_FREE(index)) && (ms_max_subnets == add_index))
        {
    ...
    if (ms_max_subnets != add_index)
    {
        if (MS_ACCESS_CONFIG_NETKEY_ADD_OPCODE == opcode)
        {
            ...
            bitarray_reset_all(ms_subnet_table[add_index].config.appkey_bitarray, ms_max_apps);
            bitarray_reset_all(ms_subnet_table[add_index].config.friend_bitarray, ms_max_lpns);
...
)
{
    ...
    ret = MS_trn_get_frndship_info
          (
              ms_friend_role,
              friend_subnet_handle - ms_max_subnets,
              &recipe
          );
...
)
{
    ...
    MS_LOOP_FOREVER()
    {
        ...
        (
            (MS_SUBNET_HANDLE)(next + ms_max_subnets),
            subnet_handle,
            key_refresh_phase
        );
        start = next + 1;
...
)
{
    ...
    if (API_SUCCESS == retval)
    {
        ...
        handle = ms_max_subnets + friend_index;
        /* Create the P data with Friendship data */
        p[0] = 0x01;
...
)
{
    ...
    retval = MS_access_cm_delete_netkey((MS_SUBNET_HANDLE)(ms_max_subnets + friend_index));
    return retval;
}
...
)
{
    ...
    else
    {
        ...
        while (index < (UINT32)(ms_max_subnets + ms_max_lpns))
        {
            if (nid == ms_subnet_table[index].fixed.nid[0])
            {
                ACCESS_TRC(
...
)
{
    ...
    /* Initialize Non-Virtual Address Table */
    EM_mem_set(ms_non_virtual_addr_table, 0, (sizeof(MS_NON_VIRTUAL_ADDR_ENTRY) * ms_max_non_virtual_addrs));
    /* Initialize Virtual Address Table */
    EM_mem_set(ms_virtual_addr_table, 0, (sizeof(MS_VIRTUAL_ADDR_ENTRY) * ms_max_virtual_addrs));
    /* Initialize Network/Relay Tx States */
    EM_mem_set(ms_tx_state, 0, sizeof(ms_tx_state));
...
)
{
    ...
    /* Parameter Validation */
    if (((ms_max_subnets + ms_max_lpns) <= handle) || MS_IS_SUBNET_FREE(handle))
    {
        retval = ACCESS_INVALID_HANDLE;
    }
    else if (NULL == nid)
...
)
{
    ...
    /* Parameter Validation */
    if ((ms_max_subnets + ms_max_lpns) <= handle)
    {
        retval = ACCESS_INVALID_HANDLE;
    }
...
API_RESULT MS_access_cm_reset(UINT8 role)
{
    ...
    if(role == PROV_ROLE_DEVICE)
    {
        ...
        for (index = 0; index < ms_access_model_count; index++)
        {
            ...
            (
                ...
                0,
                (sizeof(UINT32) * (BITARRAY_NUM_BLOCKS(ms_max_apps)))
            );
            EM_mem_set
            (
        ...
        (
            &ms_subnet_table[0],
            0,
            (sizeof(MS_NETKEY_ENTRY) * (ms_max_subnets + ms_max_lpns))
        );
        ms_access_cm_init_subnet_table();
        ...
        (
            &ms_appkey_table[0],
            0,
            (sizeof(MS_APPKEY_ENTRY) * ms_max_apps)
        );
        /* Clear Element Address Table */
        {
            ...
            (
                ...
                0,
                (sizeof(MS_ELEMENT_ADDR_ENTRY) * (1 + ms_max_lpns))
            );
        ...
        (
            ...
            0,
            (sizeof(MS_VIRTUAL_ADDR_ENTRY) * ms_max_virtual_addrs)
        );
        ...
        (
            ...
            0,
            (sizeof(MS_NON_VIRTUAL_ADDR_ENTRY) * ms_max_non_virtual_addrs)
        );
        /* Clear Tx States */
        EM_mem_set
        (
    ...
    (
        &ms_dev_key_table[0],
        0,
        (sizeof(MS_DEV_KEY_ENTRY) * ms_max_dev_keys)
    );
...
)
{
    ...
    for (index = 0; index < ms_max_apps; index ++)
    {
        ...
        else if (ms_max_apps == free_index)
        {
...
)
{
    ...
    /* Check if match found */
    if ((ms_max_apps != index) && (API_SUCCESS == retval))
    {
...
)
{
    ...
    /* Check model is valid */
    if (((MS_TRUE != publish_info->remote) || (ms_max_apps != index)) && (1 == ms_access_model_list[model_handle].fixed.valid))
    {
...
)
{
    ...
    free_index = ms_max_non_virtual_addrs;
    retval = ACCESS_NO_RESOURCE;
    ACCESS_TRC(
    ...
    for (index = 0; index < ms_max_non_virtual_addrs; index ++)
    {
        /* Check for free entry */
        if (MS_NET_ADDR_UNASSIGNED == ms_non_virtual_addr_table[index].vaddr)
        {
            if (ms_max_non_virtual_addrs == free_index)
            {
    ...
    if (ms_max_non_virtual_addrs != index)
    {
        *handle = (MS_ACCESS_ADDRESS_HANDLE)index;
        /* Return Success */
        retval = API_SUCCESS;
    }
    else if (ms_max_non_virtual_addrs != free_index)
    {
        /* Found a free entry */
...
)
{
    ...
    if (ms_max_non_virtual_addrs > addr_handle)
    {
        ACCESS_TRC(
...
//        ms_access_ps_store(MS_PS_RECORD_NON_VIRTUAL_ADDRS);
    ...
    else if ((MS_ACCESS_ADDRESS_HANDLE)MS_MAX_ADDRS > addr_handle)
    {
        ...
        addr_handle -= ms_max_non_virtual_addrs;
        ACCESS_TRC(
...
)
{
    ...
    free_index = ms_max_virtual_addrs;
    retval = ACCESS_NO_RESOURCE;

    /* Check if the label is already present in the virtual table */
    for (index = 0; index < ms_max_virtual_addrs; index ++)
    {
        /* Check for free entry */
        if (0 == ms_virtual_addr_table[index].vaddr)
        {
            if (ms_max_virtual_addrs == free_index)
            {
                /* Save first free index */
                free_index = index;
            }
    ...
    if (ms_max_virtual_addrs != index)
    {
        *handle = (MS_ACCESS_ADDRESS_HANDLE)(index + ms_max_non_virtual_addrs);
        *addr = ms_virtual_addr_table[index].vaddr;
        /* Return Success */
        retval = API_SUCCESS;
    }
    else if (ms_max_virtual_addrs != free_index)
    {
        /* Found a free entry */
        ...
        *handle = (MS_ACCESS_ADDRESS_HANDLE)(free_index + ms_max_non_virtual_addrs);
        ACCESS_TRC(
...
void ms_delete_appkey(/* IN */ UINT32 key_index)
{
    ...
    for (m_index = 0; m_index < ms_access_model_count; m_index++)
    {
