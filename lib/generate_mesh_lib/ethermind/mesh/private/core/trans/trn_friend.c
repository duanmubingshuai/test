...
)
{
    ...
    if (MS_NET_ADDR_UNASSIGNED != frnd_element.addr)
    {
        ...
        /* Send the packet with TTL 0 */
        retval = trn_frnd_send(frnd_element.addr, ms_max_subnets, action, &frnd_manage, 0);
...
)
{
    ...
    /*mark rev update pkt use Highest bit*/
    frnd_element.fsn |= 0x80;
...
void trn_frndpoll(void)
{
    ...
    trn_frnd_send(frnd_element.addr, ms_max_subnets, MS_TRN_CTRL_OPCODE_FRND_POLL, &frnd_poll, 0);
    MS_brr_sleep();
...
void trn_frndpoll_send_handler(void* args, UINT16 size)
{
    ...
    if (frnd_element.sfsn == frnd_element.fsn)
    {
        ...
        else if ((MS_FRND_POLL_RETRY_COUNT + 1) < frnd_element.poll_retry_count)
        {
            ...
            /* Update current friendship role as INVALID */
            MS_access_cm_set_friendship_role(MS_FRND_ROLE_INVALID);
            frnd_addr = frnd_element.addr;
            /* Reset the Friend Element */
            EM_mem_set(&frnd_element, 0x00, sizeof(TRN_FRND_ELEMENT));
            frnd_element.thandle = EM_TIMER_HANDLE_INIT_VAL;
            /* Reset the friend address */
            frnd_element.addr = frnd_addr;
...
)
{
    ...
    for (index = 0; index < ms_max_lpns; index ++)
    {
...
)
{
    ...
    lpn_index = subnet_handle - ms_max_subnets;
    ...
    /*rxdelay can adjust to +80 tx prepare need 20ms*/
    retval = EM_start_timer
             (
                 &lpn->thandle,
                 (EM_TIMEOUT_MILLISEC | (lpn->rx_delay+80)),
...
void trn_frndpoll_rsp_handler (void* args, UINT16 size)
{
    ...
    */
    ...
    else
    {
        ...
        {
            /* Send Network Packet */
            subnet_handle = (lpn_index + ms_max_subnets);
            buffer.payload = pdu;
...
)
{
    ...
    UINT32 index, sub_list_index;
    UINT8* paddr;
    UINT16 queue_start_store;
    UINT16 queue_start_store2;
    UINT16 queue_size_store;
        ...
        for (index = 0; index < param->num_addr; index++)
        {
            ...
            for (sub_list_index = 0; sub_list_index < MS_CONFIG_LIMITS(MS_FRIEND_SUBSCRIPTION_LIST_SIZE); sub_list_index++)
            {
                ...
                if (lpn->subscription_list[sub_list_index] == addr)
                {
                    ...
                    /*Check if friend queue has the subscription message , remove it*/
                    queue_start_store = lpn->friend_queue[0].queue_start;
                    queue_start_store2 = lpn->friend_queue[0].queue_start;
                    queue_size_store = lpn->friend_queue[0].queue_size;

                    if ((0 != lpn->friend_queue[0].queue_size))
                    {
                        for(int friend_queue_size = 0; friend_queue_size < lpn->friend_queue[0].queue_size; friend_queue_size++)
                        {
                            if(lpn->friend_queue[0].queue[lpn->friend_queue[0].queue_start].net_hdr.daddr == addr)
                            {
                                lpn->friend_queue[0].queue_size --;
                                queue_start_store2 = lpn->friend_queue[0].queue_start;

                                for(int friend_queue_size2 = 0; friend_queue_size2 < (queue_start_store + queue_size_store - lpn->friend_queue[0].queue_start); friend_queue_size2++)
                                {
                                    if(lpn->friend_queue[0].queue_start < (MS_FRIEND_MESSAGEQUEUE_SIZE -1))
                                    {
                                        lpn->friend_queue[0].queue[lpn->friend_queue[0].queue_start].net_hdr = lpn->friend_queue[0].queue[lpn->friend_queue[0].queue_start+1].net_hdr ;
                                        EM_mem_copy
                                        (
                                            lpn->friend_queue[0].queue[lpn->friend_queue[0].queue_start].pdu,
                                            lpn->friend_queue[0].queue[lpn->friend_queue[0].queue_start+1].pdu,
                                            lpn->friend_queue[0].queue[lpn->friend_queue[0].queue_start+1].ltrn_pkt_length
                                        );
                                    }
                                    else if(lpn->friend_queue[0].queue_start == (MS_FRIEND_MESSAGEQUEUE_SIZE -1))
                                    {
                                        lpn->friend_queue[0].queue[lpn->friend_queue[0].queue_start].net_hdr = lpn->friend_queue[0].queue[MS_FRIEND_MESSAGEQUEUE_SIZE - lpn->friend_queue[0].queue_start - 1].net_hdr ;
                                        EM_mem_copy
                                        (
                                            lpn->friend_queue[0].queue[lpn->friend_queue[0].queue_start].pdu,
                                            lpn->friend_queue[0].queue[MS_FRIEND_MESSAGEQUEUE_SIZE - lpn->friend_queue[0].queue_start - 1].pdu,
                                            lpn->friend_queue[0].queue[MS_FRIEND_MESSAGEQUEUE_SIZE - lpn->friend_queue[0].queue_start - 1].ltrn_pkt_length
                                        );
                                    }
                                    else
                                    {
                                        lpn->friend_queue[0].queue[lpn->friend_queue[0].queue_start - MS_FRIEND_MESSAGEQUEUE_SIZE].net_hdr = lpn->friend_queue[0].queue[lpn->friend_queue[0].queue_start - MS_FRIEND_MESSAGEQUEUE_SIZE - 1].net_hdr ;
                                        EM_mem_copy
                                        (
                                            lpn->friend_queue[0].queue[lpn->friend_queue[0].queue_start - MS_FRIEND_MESSAGEQUEUE_SIZE].pdu,
                                            lpn->friend_queue[0].queue[lpn->friend_queue[0].queue_start - MS_FRIEND_MESSAGEQUEUE_SIZE - 1].pdu,
                                            lpn->friend_queue[0].queue[lpn->friend_queue[0].queue_start - MS_FRIEND_MESSAGEQUEUE_SIZE - 1].ltrn_pkt_length
                                        );
                                    }

                                    lpn->friend_queue[0].queue_start ++;
                                }

                                lpn->friend_queue[0].queue_start = queue_start_store2;
                            }
                            else
                            {
                                lpn->friend_queue[0].queue_start ++;
                        ...
                        lpn->friend_queue[0].queue_start = queue_start_store;
                    }
