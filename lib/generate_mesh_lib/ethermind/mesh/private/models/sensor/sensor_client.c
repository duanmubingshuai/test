...
static DECL_CONST UINT32 sensor_client_opcode_list[] =
{
...
static MS_ACCESS_MODEL_HANDLE       sensor_client_model_handle;
static MS_SENSOR_CLIENT_CB          sensor_client_appl_cb;
...
)
{
    ...
    /* Configure Model */
    model.model_id.id = MS_MODEL_ID_SENSOR_CLIENT;
    model.model_id.type = MS_ACCESS_MODEL_TYPE_SIG;
    model.elem_handle = element_handle;
    /* Register Callbacks */
    model.cb = sensor_client_cb;
    model.pub_cb = NULL;
    /* List of Opcodes */
    model.opcodes = sensor_client_opcode_list;
    model.num_opcodes = sizeof(sensor_client_opcode_list) / sizeof(UINT32);
    ...
    sensor_client_appl_cb = appl_cb;
    /* TODO: Remove */
    sensor_client_model_handle = *model_handle;
    return retval;
}
...
{
    /* Fill Model Handle */
    * model_handle = sensor_client_model_handle;
    return API_SUCCESS;
}

/* Empty Model Opcode Handler Defines */
MODEL_OPCODE_HANDLER_EMPTY_DEF(sensor_register_status_handler);
MODEL_OPCODE_HANDLER_EMPTY_DEF(sensor_status_handler);
...
)
{
    ...
    if (NULL != sensor_client_appl_cb)
    {
        sensor_client_appl_cb(handle, opcode, data_param, data_len);
    }
