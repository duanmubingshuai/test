...
static DECL_CONST UINT32 light_lightness_setup_server_opcode_list[] =
{
...
static MS_ACCESS_MODEL_HANDLE   light_lightness_setup_server_model_handle;
static MS_LIGHT_LIGHTNESS_SETUP_SERVER_CB       light_lightness_setup_server_appl_cb;
...
)
{
    ...
    LIGHT_LIGHTNESS_SETUP_SERVER_TRC(
        "[LIGHT_LIGHTNESS_SETUP] Registered Element Handle 0x%02X\n", element_handle);
    /* Configure Model */
    model.model_id.id = MS_MODEL_ID_LIGHT_LIGHTNESS_SETUP_SERVER;
    model.model_id.type = MS_ACCESS_MODEL_TYPE_SIG;
    model.elem_handle = element_handle;
    /* Register Callbacks */
    model.cb = light_lightness_setup_server_cb;
    model.pub_cb = light_lightness_setup_server_publish_timout_cb;
    /* List of Opcodes */
    model.opcodes = light_lightness_setup_server_opcode_list;
    model.num_opcodes = sizeof(light_lightness_setup_server_opcode_list) / sizeof(UINT32);
    ...
    light_lightness_setup_server_appl_cb = appl_cb;
    /* TODO: Remove */
    light_lightness_setup_server_model_handle = *model_handle;
    return retval;
}
...
)
{
    ...
    LIGHT_LIGHTNESS_SETUP_SERVER_TRC(
        "[LIGHT_LIGHTNESS_SETUP_SERVER] State Update.\n");
    ...
    default:
    {
        LIGHT_LIGHTNESS_SETUP_SERVER_ERR(
...
MODEL_OPCODE_HANDLER_EMPTY_DEF(light_lightness_default_set_handler)
MODEL_OPCODE_HANDLER_EMPTY_DEF(light_lightness_range_set_handler)
...
*/
API_RESULT light_lightness_setup_server_cb
(
...
)
{
    ...
    LIGHT_LIGHTNESS_SETUP_SERVER_TRC(
        "[LIGHT_LIGHTNESS_SETUP_SERVER] Callback. Opcode 0x%04X\n", opcode);
    LIGHT_LIGHTNESS_SETUP_SERVER_debug_dump_bytes(data_param, data_len);

    switch(opcode)
    {
    case MS_ACCESS_LIGHT_LIGHTNESS_DEFAULT_SET_UNACKNOWLEDGED_OPCODE: /* Fall Through */
    case MS_ACCESS_LIGHT_LIGHTNESS_DEFAULT_SET_OPCODE:
    {
        MODEL_OPCODE_HANDLER_CALL(light_lightness_default_set_handler);
        /* Set Request Type */
        req_type.type = MS_ACCESS_MODEL_REQ_MSG_T_SET;

        if (MS_ACCESS_LIGHT_LIGHTNESS_DEFAULT_SET_OPCODE == opcode)
        {
            LIGHT_LIGHTNESS_SETUP_SERVER_TRC(
                "MS_ACCESS_LIGHT_LIGHTNESS_DEFAULT_SET_OPCODE\n");
        ...
        /* Decode Parameters */
        marker = 0;
        MS_UNPACK_LE_2_BYTE(&param.light_lightness_default.lightness_default, &data_param[marker]);
        marker += 2;
        LIGHT_LIGHTNESS_SETUP_SERVER_TRC(
            "lightness_default 0x%04X\n", param.lightness_default);
    ...
    case MS_ACCESS_LIGHT_LIGHTNESS_RANGE_SET_UNACKNOWLEDGED_OPCODE: /* Fall Through */
    case MS_ACCESS_LIGHT_LIGHTNESS_RANGE_SET_OPCODE:
    {
        MODEL_OPCODE_HANDLER_CALL(light_lightness_range_set_handler);
        /* Set Request Type */
        req_type.type = MS_ACCESS_MODEL_REQ_MSG_T_SET;

        if (MS_ACCESS_LIGHT_LIGHTNESS_RANGE_SET_OPCODE == opcode)
        {
            LIGHT_LIGHTNESS_SETUP_SERVER_TRC(
                "MS_ACCESS_LIGHT_LIGHTNESS_RANGE_SET_OPCODE\n");
            req_type.to_be_acked = 0x01;
        ...
        /* Decode Parameters */
        marker = 0;
        MS_UNPACK_LE_2_BYTE(&param.light_lightness_range.lightness_range_min, &data_param[marker]);
        marker += 2;
        MS_UNPACK_LE_2_BYTE(&param.light_lightness_range.lightness_range_max, &data_param[marker]);
        marker += 2;
        LIGHT_LIGHTNESS_SETUP_SERVER_TRC(
            "lightness_range_min 0x%04X\n", param.lightness_range_min);
        LIGHT_LIGHTNESS_SETUP_SERVER_TRC(
            "lightness_range_max 0x%04X\n", param.lightness_range_max);

        /* Parameter Validation */
        if (param.light_lightness_range.lightness_range_min > param.light_lightness_range.lightness_range_max)
        {
    ...
    if (NULL != light_lightness_setup_server_appl_cb)
    {
        light_lightness_setup_server_appl_cb(&req_context, &req_raw, &req_type, &state_params, ext_params_p);
    }
...
*/
API_RESULT light_lightness_setup_server_publish_timout_cb
(
    /* IN */ MS_ACCESS_MODEL_HANDLE* handle,
    /* IN */ void*                    blob
)
