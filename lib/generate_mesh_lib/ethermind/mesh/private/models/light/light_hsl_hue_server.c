...
static DECL_CONST UINT32 light_hsl_hue_server_opcode_list[] =
{
...
static MS_ACCESS_MODEL_HANDLE   light_hsl_hue_server_model_handle;
static MS_LIGHT_HSL_HUE_SERVER_CB       light_hsl_hue_server_appl_cb;
...
)
{
    ...
    LIGHT_HSL_HUE_SERVER_TRC(
        "[LIGHT_HSL_HUE] Registered Element Handle 0x%02X\n", element_handle);
    /* Configure Model */
    model.model_id.id = MS_MODEL_ID_LIGHT_HSL_HUE_SERVER;
    model.model_id.type = MS_ACCESS_MODEL_TYPE_SIG;
    model.elem_handle = element_handle;
    /* Register Callbacks */
    model.cb = light_hsl_hue_server_cb;
    model.pub_cb = light_hsl_hue_server_publish_timout_cb;
    /* List of Opcodes */
    model.opcodes = light_hsl_hue_server_opcode_list;
    model.num_opcodes = sizeof(light_hsl_hue_server_opcode_list) / sizeof(UINT32);
    ...
    light_hsl_hue_server_appl_cb = appl_cb;
    /* TODO: Remove */
    light_hsl_hue_server_model_handle = *model_handle;
    return retval;
}
...
)
{
    ...
    marker = 0;
    LIGHT_HSL_HUE_SERVER_TRC(
        "[LIGHT_HSL_HUE_SERVER] State Update.\n");
    ...
    case MS_STATE_LIGHT_HSL_HUE_T:
    {
        ...
        /* Set Opcode */
        opcode = MS_ACCESS_LIGHT_HSL_HUE_STATUS_OPCODE;
    }
    break;

    default:
    {
        LIGHT_HSL_HUE_SERVER_ERR(
...
MODEL_OPCODE_HANDLER_EMPTY_DEF(light_hsl_hue_get_handler)
MODEL_OPCODE_HANDLER_EMPTY_DEF(light_hsl_hue_set_handler)
...
*/
API_RESULT light_hsl_hue_server_cb
(
...
)
{
    ...
    LIGHT_HSL_HUE_SERVER_TRC(
        "[LIGHT_HSL_HUE_SERVER] Callback. Opcode 0x%04X\n", opcode);
    LIGHT_HSL_HUE_SERVER_debug_dump_bytes(data_param, data_len);
    ...
    case MS_ACCESS_LIGHT_HSL_HUE_GET_OPCODE:
    {
        LIGHT_HSL_HUE_SERVER_TRC(
            "MS_ACCESS_LIGHT_HSL_HUE_GET_OPCODE\n");
        MODEL_OPCODE_HANDLER_CALL(light_hsl_hue_get_handler);
        /* Get Request Type */
        req_type.type = MS_ACCESS_MODEL_REQ_MSG_T_GET;
        req_type.to_be_acked = 0x01;
        /* Assign reqeusted state type to the application */
        state_params.state_type = MS_STATE_LIGHT_HSL_HUE_T;
    }
    break;

    case MS_ACCESS_LIGHT_HSL_HUE_SET_UNACKNOWLEDGED_OPCODE: /* Fall Through */
    case MS_ACCESS_LIGHT_HSL_HUE_SET_OPCODE:
    {
        MODEL_OPCODE_HANDLER_CALL(light_hsl_hue_set_handler);
        /* Set Request Type */
        req_type.type = MS_ACCESS_MODEL_REQ_MSG_T_SET;

        if (MS_ACCESS_LIGHT_HSL_HUE_SET_OPCODE == opcode)
        {
            LIGHT_HSL_HUE_SERVER_TRC(
                "MS_ACCESS_LIGHT_HSL_HUE_SET_OPCODE\n");
            req_type.to_be_acked = 0x01;
        ...
        marker += 1;
        LIGHT_HSL_HUE_SERVER_TRC("hsl_hue 0x%04X. TID:0x%02X\n", param.hsl_hue, param.tid);
    ...
    if (NULL != light_hsl_hue_server_appl_cb)
    {
        light_hsl_hue_server_appl_cb(&req_context, &req_raw, &req_type, &state_params, ext_params_p);
    }
...
*/
API_RESULT light_hsl_hue_server_publish_timout_cb
(
    /* IN */ MS_ACCESS_MODEL_HANDLE* handle,
    /* IN */ void*                    blob
)
