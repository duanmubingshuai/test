...
static DECL_CONST UINT32 light_ctl_temperature_server_opcode_list[] =
{
...
static MS_ACCESS_MODEL_HANDLE   light_ctl_temperature_server_model_handle;
static MS_LIGHT_CTL_TEMPERATURE_SERVER_CB       light_ctl_temperature_server_appl_cb;
...
)
{
    ...
    LIGHT_CTL_TEMPERATURE_SERVER_TRC(
        "[LIGHT_CTL_TEMPERATURE] Registered Element Handle 0x%02X\n", element_handle);
    /* Configure Model */
    model.model_id.id = MS_MODEL_ID_LIGHT_CTL_TEMPERATURE_SERVER;
    model.model_id.type = MS_ACCESS_MODEL_TYPE_SIG;
    model.elem_handle = element_handle;
    /* Register Callbacks */
    model.cb = light_ctl_temperature_server_cb;
    model.pub_cb = light_ctl_temperature_server_publish_timout_cb;
    /* List of Opcodes */
    model.opcodes = light_ctl_temperature_server_opcode_list;
    model.num_opcodes = sizeof(light_ctl_temperature_server_opcode_list) / sizeof(UINT32);
    ...
    light_ctl_temperature_server_appl_cb = appl_cb;
    /* TODO: Remove */
    light_ctl_temperature_server_model_handle = *model_handle;
    return retval;
}
...
)
{
    ...
    marker = 0;
    LIGHT_CTL_TEMPERATURE_SERVER_TRC(
        "[LIGHT_CTL_TEMPERATURE_SERVER] State Update.\n");
    ...
    case MS_STATE_LIGHT_CTL_TEMPERATURE_T:
    {
        ...
        param_p = (MS_STATE_LIGHT_CTL_TEMPERATURE_STRUCT*)current_state_params->state;
        ...
        if (0x00 != param_p->transition_time)
        {
            ...
            marker += 2;
            MS_PACK_LE_2_BYTE_VAL(&buffer[marker], param_p->target_ctl_delta_uv);
            marker += 2;
        ...
        /* Set Opcode */
        opcode = MS_ACCESS_LIGHT_CTL_TEMPERATURE_STATUS_OPCODE;
    }
    break;

    default:
    {
        LIGHT_CTL_TEMPERATURE_SERVER_ERR(
...
MODEL_OPCODE_HANDLER_EMPTY_DEF(light_ctl_temperature_set_handler)
MODEL_OPCODE_HANDLER_EMPTY_DEF(light_ctl_temperature_get_handler)
...
*/
API_RESULT light_ctl_temperature_server_cb
(
...
)
{
    ...
    LIGHT_CTL_TEMPERATURE_SERVER_TRC(
        "[LIGHT_CTL_TEMPERATURE_SERVER] Callback. Opcode 0x%04X\n", opcode);
    LIGHT_CTL_TEMPERATURE_SERVER_debug_dump_bytes(data_param, data_len);
    ...
    case MS_ACCESS_LIGHT_CTL_TEMPERATURE_GET_OPCODE:
    {
        LIGHT_CTL_TEMPERATURE_SERVER_TRC(
            "MS_ACCESS_LIGHT_CTL_TEMPERATURE_GET_OPCODE\n");
        MODEL_OPCODE_HANDLER_CALL(light_ctl_temperature_get_handler);
        /* Get Request Type */
        req_type.type = MS_ACCESS_MODEL_REQ_MSG_T_GET;
        req_type.to_be_acked = 0x01;
        /* Assign reqeusted state type to the application */
        state_params.state_type = MS_STATE_LIGHT_CTL_TEMPERATURE_T;
    }
    break;

    case MS_ACCESS_LIGHT_CTL_TEMPERATURE_SET_UNACKNOWLEDGED_OPCODE: /* Fall Through */
    case MS_ACCESS_LIGHT_CTL_TEMPERATURE_SET_OPCODE:
    {
        MODEL_OPCODE_HANDLER_CALL(light_ctl_temperature_set_handler);
        /* Set Request Type */
        req_type.type = MS_ACCESS_MODEL_REQ_MSG_T_SET;

        if (MS_ACCESS_LIGHT_CTL_TEMPERATURE_SET_OPCODE == opcode)
        {
            LIGHT_CTL_TEMPERATURE_SERVER_TRC(
                "MS_ACCESS_LIGHT_CTL_TEMPERATURE_SET_OPCODE\n");
            req_type.to_be_acked = 0x01;
        ...
        {
            LIGHT_CTL_TEMPERATURE_SERVER_TRC("ctl_temperature 0x%04X out of range. Dropping...\n",
        ...
        LIGHT_CTL_TEMPERATURE_SERVER_TRC(
            "ctl_delta_uv 0x%04X\n", param.ctl_delta_uv);
        LIGHT_CTL_TEMPERATURE_SERVER_TRC("TID 0x%04X\n", param.tid);
    ...
    if (NULL != light_ctl_temperature_server_appl_cb)
    {
        light_ctl_temperature_server_appl_cb(&req_context, &req_raw, &req_type, &state_params, ext_params_p);
    }
...
*/
API_RESULT light_ctl_temperature_server_publish_timout_cb
(
    /* IN */ MS_ACCESS_MODEL_HANDLE* handle,
    /* IN */ void*                    blob
)
