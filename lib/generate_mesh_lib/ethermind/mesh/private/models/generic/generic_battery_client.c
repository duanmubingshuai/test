...
static DECL_CONST UINT32 generic_battery_client_opcode_list[] =
{
    MS_ACCESS_GENERIC_BATTERY_STATUS_OPCODE
};

static MS_ACCESS_MODEL_HANDLE   generic_battery_client_model_handle;
static MS_GENERIC_BATTERY_CLIENT_CB       generic_battery_client_appl_cb;
...
)
{
    ...
    GENERIC_BATTERY_CLIENT_TRC(
        "[GENERIC_BATTERY] Registered Element Handle 0x%02X\n", element_handle);
    /* Configure Model */
    model.model_id.id = MS_MODEL_ID_GENERIC_BATTERY_CLIENT;
    model.model_id.type = MS_ACCESS_MODEL_TYPE_SIG;
    model.elem_handle = element_handle;
    /* Register Callbacks */
    model.cb = generic_battery_client_cb;
    model.pub_cb = NULL;
    /* List of Opcodes */
    model.opcodes = generic_battery_client_opcode_list;
    model.num_opcodes = sizeof(generic_battery_client_opcode_list) / sizeof(UINT32);
    ...
    generic_battery_client_appl_cb = appl_cb;
    /* TODO: Remove */
    generic_battery_client_model_handle = *model_handle;
    return retval;
}
...
)
{
    ...
    marker = 0;
    GENERIC_BATTERY_CLIENT_TRC(
        "[GENERIC_BATTERY_CLIENT] Send Reliable PDU. Req Opcode 0x%08X, Rsp Opcode 0x%08X\n",
    ...
    retval = MS_access_publish
             (
                 &generic_battery_client_model_handle,
                 req_opcode,
                 pdu_ptr,
                 marker,
                 MS_TRUE
...
{
    /* Fill Model Handle */
    * model_handle = generic_battery_client_model_handle;
    return API_SUCCESS;
}

/* Empty Model Opcode Handler Defines */
MODEL_OPCODE_HANDLER_EMPTY_DEF(generic_battery_status_handler)
...
*/
API_RESULT generic_battery_client_cb
(
...
)
{
    ...
    GENERIC_BATTERY_CLIENT_debug_dump_bytes(data_param, data_len);
    ...
    case MS_ACCESS_GENERIC_BATTERY_STATUS_OPCODE:
    {
        GENERIC_BATTERY_CLIENT_TRC(
            "MS_ACCESS_GENERIC_BATTERY_STATUS_OPCODE\n");
        MODEL_OPCODE_HANDLER_CALL(generic_battery_status_handler);
    }
    break;
    }

    /* Application callback */
    if (NULL != generic_battery_client_appl_cb)
    {
        generic_battery_client_appl_cb(handle, opcode, data_param, data_len);
    }
