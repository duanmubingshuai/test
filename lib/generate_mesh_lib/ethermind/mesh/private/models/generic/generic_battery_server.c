...
static DECL_CONST UINT32 generic_battery_server_opcode_list[] =
{
    MS_ACCESS_GENERIC_BATTERY_GET_OPCODE,
};

static MS_ACCESS_MODEL_HANDLE   generic_battery_server_model_handle;
static MS_GENERIC_BATTERY_SERVER_CB       generic_battery_server_appl_cb;
...
)
{
    ...
    GENERIC_BATTERY_SERVER_TRC(
        "[GENERIC_BATTERY] Registered Element Handle 0x%02X\n", element_handle);
    /* Configure Model */
    model.model_id.id = MS_MODEL_ID_GENERIC_BATTERY_SERVER;
    model.model_id.type = MS_ACCESS_MODEL_TYPE_SIG;
    model.elem_handle = element_handle;
    /* Register Callbacks */
    model.cb = generic_battery_server_cb;
    model.pub_cb = generic_battery_server_publish_timout_cb;
    /* List of Opcodes */
    model.opcodes = generic_battery_server_opcode_list;
    model.num_opcodes = sizeof(generic_battery_server_opcode_list) / sizeof(UINT32);
    ...
    generic_battery_server_appl_cb = appl_cb;
    /* TODO: Remove */
    generic_battery_server_model_handle = *model_handle;
    return retval;
}
...
)
{
    ...
    marker = 0;
    GENERIC_BATTERY_SERVER_TRC(
        "[GENERIC_BATTERY_SERVER] State Update.\n");
    ...
    case MS_STATE_GENERIC_BATTERY_T:
    {
        ...
        param_p = (MS_STATE_GENERIC_BATTERY_STRUCT*)current_state_params->state;
        buffer[marker] = param_p->generic_battery_level;
        marker += 1;
        MS_PACK_LE_3_BYTE_VAL(&buffer[marker], param_p->generic_battery_time_to_discharge);
        marker += 3;
        MS_PACK_LE_3_BYTE_VAL(&buffer[marker], param_p->generic_battery_time_to_charge);
        marker += 3;
        buffer[marker] = param_p->generic_battery_flags;
        marker += 1;
        /* Set Opcode */
        opcode = MS_ACCESS_GENERIC_BATTERY_STATUS_OPCODE;
    }
    break;

    default:
    {
        GENERIC_BATTERY_SERVER_ERR(
...
MODEL_OPCODE_HANDLER_EMPTY_DEF(generic_battery_get_handler)
...
*/
API_RESULT generic_battery_server_cb
(
...
)
{
    ...
    GENERIC_BATTERY_SERVER_TRC(
        "[GENERIC_BATTERY_SERVER] Callback. Opcode 0x%04X\n", opcode);
    GENERIC_BATTERY_SERVER_debug_dump_bytes(data_param, data_len);

    switch(opcode)
    {
    case MS_ACCESS_GENERIC_BATTERY_GET_OPCODE:
    {
        GENERIC_BATTERY_SERVER_TRC(
            "MS_ACCESS_GENERIC_BATTERY_GET_OPCODE\n");
        MODEL_OPCODE_HANDLER_CALL(generic_battery_get_handler);
        /* Get Request Type */
        req_type.type = MS_ACCESS_MODEL_REQ_MSG_T_GET;
        req_type.to_be_acked = 0x01;
        /* Assign reqeusted state type to the application */
        state_params.state_type = MS_STATE_GENERIC_BATTERY_T;
    }
    break;
    }

    /* Application callback */
    if (NULL != generic_battery_server_appl_cb)
    {
        generic_battery_server_appl_cb(&req_context, &req_raw, &req_type, &state_params, ext_params_p);
    }
...
*/
API_RESULT generic_battery_server_publish_timout_cb
(
    /* IN */ MS_ACCESS_MODEL_HANDLE* handle,
    /* IN */ void*                    blob
)
