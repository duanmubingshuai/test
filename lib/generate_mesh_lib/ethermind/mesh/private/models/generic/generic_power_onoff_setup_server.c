...
static DECL_CONST UINT32 generic_power_onoff_setup_server_opcode_list[] =
{
    MS_ACCESS_GENERIC_ONPOWERUP_SET_OPCODE,
    MS_ACCESS_GENERIC_ONPOWERUP_SET_UNACKNOWLEDGED_OPCODE,
};

static MS_ACCESS_MODEL_HANDLE   generic_power_onoff_setup_server_model_handle;
static MS_GENERIC_POWER_ONOFF_SETUP_SERVER_CB       generic_power_onoff_setup_server_appl_cb;
...
)
{
    ...
    GENERIC_POWER_ONOFF_SETUP_SERVER_TRC(
        "[GENERIC_POWER_ONOFF_SETUP] Registered Element Handle 0x%02X\n", element_handle);
    /* Configure Model */
    model.model_id.id = MS_MODEL_ID_GENERIC_POWER_ONOFF_SETUP_SERVER;
    model.model_id.type = MS_ACCESS_MODEL_TYPE_SIG;
    model.elem_handle = element_handle;
    /* Register Callbacks */
    model.cb = generic_power_onoff_setup_server_cb;
    model.pub_cb = generic_power_onoff_setup_server_publish_timout_cb;
    /* List of Opcodes */
    model.opcodes = generic_power_onoff_setup_server_opcode_list;
    model.num_opcodes = sizeof(generic_power_onoff_setup_server_opcode_list) / sizeof(UINT32);
    ...
    generic_power_onoff_setup_server_appl_cb = appl_cb;
    /* TODO: Remove */
    generic_power_onoff_setup_server_model_handle = *model_handle;
    return retval;
}
...
)
{
    ...
    marker = 0;
    GENERIC_POWER_ONOFF_SETUP_SERVER_TRC(
        "[GENERIC_POWER_ONOFF_SETUP_SERVER] State Update.\n");
    ...
    case MS_STATE_GENERIC_ONPOWERUP_T:
    {
        ...
        param_p = (MS_STATE_GENERIC_ONPOWERUP_STRUCT*)current_state_params->state;
        buffer[marker] = param_p->onpowerup;
        marker += 1;
        /* Set Opcode */
        opcode = MS_ACCESS_GENERIC_ONPOWERUP_STATUS_OPCODE;
    }
    break;

    default:
    {
        GENERIC_POWER_ONOFF_SETUP_SERVER_ERR(
...
MODEL_OPCODE_HANDLER_EMPTY_DEF(generic_onpowerup_set_handler)
...
*/
API_RESULT generic_power_onoff_setup_server_cb
(
...
)
{
    ...
    GENERIC_POWER_ONOFF_SETUP_SERVER_TRC(
        "[GENERIC_POWER_ONOFF_SETUP_SERVER] Callback. Opcode 0x%04X\n", opcode);
    GENERIC_POWER_ONOFF_SETUP_SERVER_debug_dump_bytes(data_param, data_len);

    switch(opcode)
    {
    case MS_ACCESS_GENERIC_ONPOWERUP_SET_UNACKNOWLEDGED_OPCODE: /* Fall Through */
    case MS_ACCESS_GENERIC_ONPOWERUP_SET_OPCODE:
    {
        MODEL_OPCODE_HANDLER_CALL(generic_onpowerup_set_handler);
        /* Set Request Type */
        req_type.type = MS_ACCESS_MODEL_REQ_MSG_T_SET;

        if (MS_ACCESS_GENERIC_ONPOWERUP_SET_OPCODE == opcode)
        {
            GENERIC_POWER_ONOFF_SETUP_SERVER_TRC(
                "MS_ACCESS_GENERIC_ONPOWERUP_SET_OPCODE\n");
        ...
        /* Decode Parameters */
        marker = 0;
        param.onpowerup = data_param[marker];
        marker += 1;
        GENERIC_POWER_ONOFF_SETUP_SERVER_TRC(
            "onpowerup 0x%02X\n", param.onpowerup);

        /* Parameter Validation */
        if (0x02 < param.onpowerup)
        {
            GENERIC_POWER_ONOFF_SETUP_SERVER_TRC(
                "onpowerup 0x%02X\n", param.onpowerup);
        ...
        state_params.state_type = MS_STATE_GENERIC_ONPOWERUP_T;
    ...
    if (NULL != generic_power_onoff_setup_server_appl_cb)
    {
        generic_power_onoff_setup_server_appl_cb(&req_context, &req_raw, &req_type, &state_params, ext_params_p);
    }
...
*/
API_RESULT generic_power_onoff_setup_server_publish_timout_cb
(
    /* IN */ MS_ACCESS_MODEL_HANDLE* handle,
    /* IN */ void*                    blob
)
