 ...
 *          bleMemAllocError: Memory allocation error occurred.
 */
bStatus_t l2capEncapSendData0( uint16 connHandle, l2capPacket_t *pPkt )
{
  uint8 *pBuf;
  uint8 *pHdr;
  uint8 totalLen;
//#ifdef _PHY_DEBUG 
//			LOG("%s,%s,Line %d,connHandle 0x%X\n",__FILE__,__func__,__LINE__,connHandle);
//	#endif
...
//        }
		status = HCI_SendDataPkt( connHandle, LL_DATA_FIRST_PKT_HOST_TO_CTRL, totalLen, pBuf );
...
bStatus_t l2capNotifyData( uint8 taskId, uint16 connHandle, l2capPacket_t *pPkt )
{
  ...
  if ( pMsg != NULL )
  {
    ...
    if ( pPkt != NULL )
    {
      osal_memcpy( &(pMsg->pkt), pPkt, sizeof( l2capPacket_t ) );
    }
    else
    {
      osal_memset( &(pMsg->pkt), 0, sizeof( l2capPacket_t ) );
    }
...
{
  // Stop the timer
  osal_CbTimerStop( pChannel->timerId );
...
static bStatus_t L2CAP_SendDataPkt( uint16 connHandle, uint16 pktLen, uint8 *pData )
{
  ...
  if ( pMsg != NULL )
  {
    ...
    osal_msg_send( l2capTaskID, (uint8 *)pMsg );
...
//uint8 L2CAP_Fragment_SendDataPkt( uint16 connHandle, uint8 fragFlg,uint16 pktLen, uint8 *pBuf )
//{
