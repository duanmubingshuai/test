...
gapAuthStateParams_t *pAuthLink = NULL;
...
static void gapFreeAuthLink( void );
...
bStatus_t GAP_Authenticate( gapAuthParams_t *pParams, gapPairingReq_t *pPairReq )
{
  ...
  pAuthLink = (gapAuthStateParams_t *)osal_mem_alloc( (uint16)(sizeof ( gapAuthStateParams_t )) );
  LOG("%s,osal mem alloc pAuthLink %p\n",__func__,pAuthLink);
  if ( pAuthLink == NULL )
  {
    return ( bleMemAllocError );
  }

  VOID osal_memset( pAuthLink, 0, (int)(sizeof( gapAuthStateParams_t )) );

  // Copy existing items
  pAuthLink->connectionHandle = pParams->connectionHandle;
  VOID osal_memcpy( &(pAuthLink->secReqs), &(pParams->secReqs), sizeof ( smLinkSecurityReq_t ) );

  // Start authentication
  pAuthLink->state = AUTHSTATE_PAIRING;
...
//  gapConnectedCleanUpAdvertising();
  ...
  if ( pPkt->status == SUCCESS )
  {
    ...
    pPkt->status = linkDB_Add( appTaskID, pPkt->connectionHandle, LINK_CONNECTED,
                       ...
                       uint16 connTimeout, uint16 clockAccuracy )
{
  ...
  if ( pRsp )
  {
    ...
    else
    {
      ...
      osal_memset( pRsp->devAddr, 0, B_ADDR_LEN );
    }
    ...
    osal_msg_send( taskID, (uint8 *)pRsp );
  }
}
                           ...
                           smSecurityInfo_t *pDevEncParams )
{
  ...
  if ( (pLink != NULL) && (pAuthLink != NULL) )
  {
    ...
    if ( pAuthLink->pSecurityInfo )
    {
      len += sizeof ( smSecurityInfo_t );
    }
    if ( pAuthLink->pSigningInfo )
    {
    ...
    if ( pRsp )
    {
      ...
      if ( pAuthLink->pSecurityInfo )
      {
        ...
        VOID osal_memcpy( pRsp->pSecurityInfo, pAuthLink->pSecurityInfo, sizeof ( smSecurityInfo_t ) );
      }
      ...
      if ( pAuthLink->pSigningInfo )
      {
        ...
        VOID osal_memcpy( pRsp->pSigningInfo, pAuthLink->pSigningInfo, sizeof ( smSigningInfo_t ) );
      }
      ...
      if ( pAuthLink->pIdentityInfo )
      {
        ...
        VOID osal_memcpy( pRsp->pIdentityInfo, pAuthLink->pIdentityInfo, sizeof ( smIdentityInfo_t ) );
      }
      
      VOID osal_msg_send( taskID, (uint8 *)pRsp );

      // Remove the pAuthLink
      gapFreeAuthLink();
    }
  }
}
 ...
 * @fn          gapFreeAuthLink
 *
 * @brief       Free the allocated memory used in pAuthLink.
 *
 * @param       none
 *
 * @return      none
...
LOG("\n%s,pAuthLink %p\n",__func__,pAuthLink);
  if ( pAuthLink )
  {
    if ( pAuthLink->pSecurityInfo )
    {
      osal_mem_free( pAuthLink->pSecurityInfo );
    }
    if ( pAuthLink->pIdentityInfo )
    {
      osal_mem_free( pAuthLink->pIdentityInfo );
    }
    if ( pAuthLink->pSigningInfo )
    {
      osal_mem_free( pAuthLink->pSigningInfo );
    }
    osal_mem_free( pAuthLink );
    pAuthLink = NULL;
  }
}
                          ...
                          smSigningInfo_t  *pSigningInfo )
{
  ...
  if ( pAuthLink )
  {
    ...
    if ( pEncParams )
    {
      if ( pAuthLink->pSecurityInfo == NULL )
      {
        pAuthLink->pSecurityInfo = (smSecurityInfo_t *)osal_mem_alloc( (uint16)(sizeof ( smSecurityInfo_t )) );
      }

      if ( pAuthLink->pSecurityInfo )
      {
        VOID osal_memcpy( pAuthLink->pSecurityInfo, pEncParams, sizeof( smSecurityInfo_t ) );
      }
    }
    ...
    if ( pIdInfo )
    {
      if (pAuthLink->pIdentityInfo == NULL )
      {
        pAuthLink->pIdentityInfo = (smIdentityInfo_t *)osal_mem_alloc( (uint16)(sizeof ( smIdentityInfo_t )) );
      }

      if ( pAuthLink->pIdentityInfo )
      {
        VOID osal_memcpy( pAuthLink->pIdentityInfo, pIdInfo, sizeof( smIdentityInfo_t ) );
      }
    }
    ...
    if ( pSigningInfo )
    {
      if ( pAuthLink->pSigningInfo == NULL )
      {
        pAuthLink->pSigningInfo = (smSigningInfo_t *)osal_mem_alloc( (uint16)(sizeof ( smSigningInfo_t )) );
      }

      if ( pAuthLink->pSigningInfo )
      {
        VOID osal_memcpy( pAuthLink->pSigningInfo, pSigningInfo, sizeof( smSigningInfo_t ) );
      }
    }
