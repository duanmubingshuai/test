...
static uint8 smResponderProcessLTKReq( uint16 connectionHandle, uint8 *pRandom, uint16 encDiv )
{
  ...
  #ifdef _PHY_DEBUG 
LOG("%s\n",__func__);
LOG("pPairingParams %p,pPairingParams->pEncParams %p,pPairingParams->state %d\n",pPairingParams,pPairingParams->pEncParams,pPairingParams->state);

  LOG("pPairingParams->myComp.rand:");
	for(uint8 i=0;i<16;i++)
		LOG("0x%02X,",pPairingParams->myComp.rand[i]);
	  LOG("\n");
	LOG("pPairingParams->devComp.rand:");
	for(uint8 i=0;i<16;i++)
		LOG("0x%02X,",pPairingParams->devComp.rand[i]);
	  LOG("\n");
        ...
        && (pPairingParams->initiator == FALSE) )
  {
    ...
    if ( pPairingParams->state == SM_PAIRING_STATE_WAIT_STK )
    {
                 ...
                 pPairingParams->devComp.rand, ltk ) == SUCCESS )
      {
        ...
        osal_memcpy( cmdLtk, ltk, smDetermineKeySize() );
...
//#ifdef _PHY_DEBUG 
		 LOG("%s,%s,Line %d,pPairingParams %p\n",__FILE__,__func__,__LINE__,pPairingParams);
//   #endif
...
#ifdef _PHY_DEBUG 
		  LOG("%s,%s,Line %d,cmdID 0x%X\n",__FILE__,__func__,__LINE__,cmdID);
	#endif
...
LOG("pPairingParams->tk:");
for(uint8 i=0;i<16;i++)
	LOG("0x%02X,",pPairingParams->tk[i]);
  LOG("\n");
#endif
  ...
  uint8 ret = sm_c1( pPairingParams->tk,
        pPairingParams->devComp.rand,
        confirm );
  #ifdef _PHY_DEBUG 
  LOG("pPairingParams->initiator %d, ret %d,pPairingParams->type %d\n",pPairingParams->initiator,ret,pPairingParams->type);


	LOG("pPairingParams->myComp.confirm:");
	 for(uint8 i=0;i<16;i++)
		 LOG("0x%02X,",pPairingParams->myComp.confirm[i]);
	   LOG("\n");
	LOG("pPairingParams->devComp.confirm:");
	 for(uint8 i=0;i<16;i++)
		 LOG("0x%02X,",pPairingParams->devComp.confirm[i]);
	   LOG("\n");
	 
	 LOG("confirm::");
	 for(uint8 i=0;i<16;i++)
		 LOG("0x%02X,",confirm[i]);
	 LOG("\n");
	 #endif
...
static uint8 smpResponderProcessEncryptionInformation( smpEncInfo_t *pParsedMsg )
{
  if ( pPairingParams->state == SM_PAIRING_STATE_WAIT_MASTER_ENCRYPTION_INFO )
  {
    ...
    {
      osal_memcpy( pPairingParams->pDevEncParams->ltk, pParsedMsg->ltk, KEYLEN );
...
//      else
      ...
      // Set up the next send
       osal_start_timerEx( smTaskID, SM_PAIRING_STATE_EVT, timeout );
    }
  }
}
