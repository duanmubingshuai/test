 ...
 *          fixed channel.
 *
 * @param   pL2capMsg 每 pointer to received L2CAP message
 * @param   pPkt 每 pointer to parsed packet
 *
 * @return  SUCCESS or FAILURE
 */
uint8 ATT_ParsePacket( l2capDataEvent_t *pL2capMsg, attPacket_t *pPkt )
{
  uint8 len = pL2capMsg->pkt.len;
...
//LOG("%s,len %d,pParams[0] %d\n",__func__,len,pParams[0]);
 ...
 *
 * @param   connHandle 每 connection to use
 * @param   pfnBuildMsg 每 build function for message
 * @param   opcode 每 operation code
 * @param   pMsg 每 pointer to message to be sent
 *
...
bStatus_t attSendMsg( uint16 connHandle, attBuildMsg_t pfnBuildMsg, uint8 opcode, uint8 *pMsg )
{
  ...
  buf = (uint8 *)L2CAP_bm_alloc( ATT_MTU_SIZE );
  if ( buf != NULL )
  {
    uint8 *pBuf = buf;
    uint16 len = 1; // opcode
