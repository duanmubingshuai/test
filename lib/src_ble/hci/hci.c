 ...
 *       and/or changed in this routine, a stack engineer must be notified!
 *
 * Public function defined in hci.h.
 */
                             ...
                             uint8  *pData )
{
	...
	return (LL_TxData( connHandle, pData, pktLen, pbFlag ));
}
...
//  HCI_CommandStatusEvent( LL_Disconnect(connHandle, reason), HCI_DISCONNECT );
	LL_Disconnect(connHandle, reason);
  return( HCI_SUCCESS );
}
