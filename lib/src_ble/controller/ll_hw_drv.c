...
const uint8_t whiten_seed[40] = {	0x1,0x41,0x21,0x61,0x11,0x51,0x31,0x71,0x9,0x49,\
							0x29,0x69,0x19,0x59,0x39,0x79,0x5,0x45,0x25,0x65,\
							0x15,0x55,0x35,0x75,0xD,0x4D,0x2D,0x6D,0x1D,0x5D,\
							0x3D,0x7D,0x3,0x43,0x23,0x63,0x13,0x53,0x33,0x73};
...
void	 ll_hw_config(uint8_t llMode,uint8_t snNesn,uint8_t txNum,uint8_t rxNum,uint8_t mdRx,uint32_t rdCntIni)
{
    	...
    	if(		 llMode==LL_HW_STX ){		ll_hw_set_stx();
    	}else if(llMode==LL_HW_SRX ){  	    ll_hw_set_srx();
    	}else if(llMode==LL_HW_TRX ){		ll_hw_set_trx();
    	}else if(llMode==LL_HW_RTX ){		ll_hw_set_rtx();
    	}else if(llMode==LL_HW_TRLP){		ll_hw_set_trlp(	    snNesn,
				   									            txNum,
				   									            rxNum,
				   									            mdRx); 	
    			   					
    	}else if(llMode==LL_HW_TRLP_EMPT){  ll_hw_set_trlp(	    snNesn,
				   								        	    txNum+1,/*add one empty pkt*/
				   								        	    rxNum,
				   								        	    mdRx); 	
    			   					
    	}else if(llMode==LL_HW_RTLP){		ll_hw_set_rtlp(	    snNesn,
				   								        	    txNum,
				   								        	    rxNum,
				   								        	    mdRx,
				   										        rdCntIni);
			
		}else if(llMode==LL_HW_RTLP_EMPT){  ll_hw_set_rtlp(	    snNesn,
				   								        	    txNum+1,/*add one empty pkt*/
				   								        	    rxNum,
				   								        	    mdRx,
				   										         /*one empty pkt*/1);
			
    	}else if(llMode==LL_HW_RTLP_1ST){	ll_hw_set_rtlp_1st( snNesn,
															    txNum,
															    rxNum,
															    mdRx);
...
//	}
	...
	//test code
//	static uint32 test_in;
//	if( llState == LL_STATE_CONN_SLAVE)
//	{
//        test_in++;
////        if((test_in>500)&&(test_in%10 !=0) )
////        {
////	        gpio_write(P24,1);
////			gpio_write(P24,0);
////            set_channel(37);
...
//	}
	...
	*(volatile uint32_t *)(LL_HW_BASE+ 0x00) = 0x0001;			//trig 
	...
	{
        ll_hw_set_rfphyFreqOff(rfChnIdx);
	}
	//2018-02-09 ZQ
...
//	if( ( llState == LL_STATE_INIT ) || ( llSecondaryState == LL_SEC_STATE_INIT) || ( llSecondaryState == LL_SEC_STATE_INIT_PENDING) )
//		gpio_write(P23,1);
//gpio_write(P23,0);
