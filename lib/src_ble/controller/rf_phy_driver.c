...
#define RF_PHY_DTM_CTRL_MOD                             RF_PHY_DTM_CTRL_NONE
...
volatile uint8_t        g_rfPhyTxPower      =   0x1f;
...
volatile rxadc_clk_t    g_rxAdcClkSel       =   RX_ADC_CLK_SEL_32M_DLL;
...
void rf_tpCal_cfg(uint8 rfChn)
{
    ...
    {
        //rfPhyTpCal0=rf_tp_cal(rfChn,0)+5;
        //ZQ:debug 20180427
...
void rf_tpCal_cfg_avg(uint8 rfChn,uint8 avgNum)
{
    ...
    for ( i=0; i<(1<<avgNum); i++)
        tmp+=rf_tp_cal(rfChn,fdev);

    g_rfPhyTpCal0=(tmp>>avgNum);
...
void rf_phy_ana_cfg(void)
{
    ...
    {
//        subWriteReg(0x4000f044, 4, 4, 0x01);  // DLL ldo
...
void rf_phy_bb_cfg(uint8_t pktFmt)
{
	...
	subWriteReg(0x40030008,8,8,0);//turn off ahdfe
    PHY_REG_WT(0x40031074,0x00800080); // set default ll fifo depth
}
...
//    while(timeOut--) {};
    if(fDev==0)
        wait_hclk_cycle_us(4000);
    else
        wait_hclk_cycle_us(2000);
...
#define CHECK_UART_WR_IDX                  {if(p_uart_wr_Idx!=0 && *p_uart_wr_Idx>=2){break;}}
...
{
    uint8_t len = 0;
    uint32_t currTick,deltTick,dtmTick;
    uint32_t pllMoniter;
    //---------------------------------------------------------------------------------------- 
    //get input param
    //
   
    uint8_t dtmModeType   =  cmdWord        &  0xff;
    uint8_t rfChn         = (cmdWord>> 8)   &  0xff;
//    uint8_t dtmLength     = ((cmdWord>>16)  &  0x0f)<<3;      
//    uint8_t dtmPKT        = (cmdWord>>20)   &  0x0f;
    uint8_t dtmLength     = (cmdWord>>16)   &  0x3f; //[21:16]      
    uint8_t dtmPKT        = (cmdWord>>22)   &  0x03; //[23:22]

    uint16_t loopN         = (cmdWord>>24)   &  0xff;        // for ll trig times

    if(loopN == 0xff)
        loopN = 0xffff;// for dtm tx rx case

    if(loopN == 0xfe)
        loopN = 1000; //  for rx per auto case
    ...
    if(         g_rfPhyPktFmt==PKT_FMT_BLE1M){
        pktLenUs = (dtmLength+1+4+2+3)<<3;
    }else if(   g_rfPhyPktFmt==PKT_FMT_BLE2M){
        pktLenUs = (dtmLength+2+4+2+3)<<2;
    }
        //ate reuse tx_scanRsp buf for tx data
    //only set the datlen, use the random data
    extern struct buf_tx_desc tx_scanRsp_desc;
    tx_scanRsp_desc.txheader=(dtmLength<<8);//len=40 same as former ate 
    
    uint16_t dtmPktIntv = (((pktLenUs+249)+624)/625)*625;//  ceil((L+249)/625) * 625
    //---------------------------------------------------------------------------------------- 
    //clear test override
    //
    PHY_REG_WT( 0x40030040,0x00030000);     // close tx_bb test mode
    ...
    if(dtmModeType == RF_PHY_DTM_MODE_ATE_SET_TXPOWER)
    {
        
        g_rfPhyTxPower      =   (cmdWord>>8) & 0x1f;
        len                 =   1;
        dOut[0]             =   g_rfPhyTxPower;
        
    }
    else if(dtmModeType == RF_PHY_DTM_MODE_ATE_SET_PKTFMT)
    {
        
        g_rfPhyPktFmt       =   (cmdWord>>8) & 0x07;
        len                 =   1;
        dOut[0]             =   dtmModeType;
        
    }
    else if(   dtmModeType == RF_PHY_DTM_MODE_ATE_TX_BURST
             || dtmModeType == RF_PHY_DTM_MODE_ATE_TX_MOD
             || dtmModeType == RF_PHY_DTM_MODE_ATE_TX_CARR   )
    {
        ...
        PHY_REG_WT(0x40030044,RF_PHY_DTM_PRBS9_SEED );
        PHY_REG_WT(0x40030048,RF_PHY_DTM_CRC_WT     );  
        PHY_REG_WT(0x4003004c,RF_PHY_DTM_SYNC_WORD  );
        
        PHY_REG_WT(0x40030040,0x00030010);
                 
        if(dtmModeType == RF_PHY_DTM_MODE_ATE_TX_CARR)
        {
        ...
        if(dtmModeType == RF_PHY_DTM_MODE_ATE_TX_BURST)
        { 
            //[15:8] payload Len [7:5] preamble len, [4] tx mode, [3:0] payload type     
            PHY_REG_WT(0x40030040,(0x00030010|(dtmLength<<8)|(preambleLen<<5)|(0x0f & dtmPKT)) );
        }
        else
        {
            //[15:8] payload Len [7:5] preamble len, [4] tx mode, [3:0] payload type     
            PHY_REG_WT(0x40030040,0x00030010|(preambleLen<<5)|(0x0f & dtmPKT));

            rf_phy_set_txPower(0x1f&(cmdWord>>24));      // for CTMOD and CARRIER set txpower
            subWriteReg(0x4000000c, 10, 10, 0);
            subWriteReg(0x4000000c, 10, 10, 1);
        ...
        // ate reg patch
        //
        for(int i=0;i<regPatchNum;i++)
        {
            if((regPatchHigh[i]==31) && (regPatchLow[i]==0)){
                PHY_REG_WT(regPatchAddr[i],regPatchVal[i]);
            }else{
                subWriteReg(regPatchAddr[i],regPatchHigh[i],regPatchLow[i],regPatchVal[i]);
			}
        }
        ...
        {
            //check test mode,
            //if ate reg patch set normal mode, use user pkt     
            if(0==(PHY_REG_RD(0x40030040)&0x10))
            {
                ll_hw_rst_tfifo();
                ll_hw_rst_rfifo();

                //ll_hw_write_tfifo(txPkt,40);
                ll_hw_write_tfifo((uint8 *)&(tx_scanRsp_desc.txheader), 
                                       ((tx_scanRsp_desc.txheader & 0xff00) >> 8) + 2); 
            }
            ll_hw_trigger();
               
            dtmTick = read_current_fine_time();
            ...
            while(loopN)
            {
                ...
                currTick = read_current_fine_time();
                deltTick = RF_PHY_TIME_DELTA(currTick,dtmTick);

                if(deltTick>=dtmPktIntv){

                    loopN = (loopN==0xFFFF) ? loopN :(loopN-1);

                    ll_hw_rst_tfifo();
                    ll_hw_rst_rfifo();

                    ll_hw_trigger();   
                    dtmTick = dtmTick+dtmPktIntv;

                    if(dtmTick>RF_PHY_TIME_BASE)
                        dtmTick = dtmTick-RF_PHY_TIME_BASE;
        ...
        else if(dtmModeType == RF_PHY_DTM_MODE_ATE_TX_CARR)
        {
            
            subWriteReg(0x40030080,14,14,0x00);//turn off lock detect to clr ld_ready bit
            
            subWriteReg(0x40030080,14,12,0x07);//turn on lock detect and ld_time is 128us
            ll_hw_trigger();
            WaitUs(((cmdWord>>16)&0xff)<<4);
            pllMoniter=(PHY_REG_RD(0x400300f8)&0x00ffffff);
            len = 4;
            dOut[0] = (pllMoniter>>16) & 0xff;
            dOut[1] = (pllMoniter>> 8) & 0xff;
            dOut[2] =  pllMoniter      & 0xff;
            dOut[3] = g_rfPhyTpCal0;
        }
        else
        {
            ll_hw_trigger();
            len = 1;
            dOut[0] = g_rfPhyTpCal0;
        }
         
        
    }
    else if(   dtmModeType == RF_PHY_DTM_MODE_ATE_RX_AUTOGAIN 
             || dtmModeType == RF_PHY_DTM_MODE_ATE_RX_FIXGAIN
             || dtmModeType == RF_PHY_DTM_MODE_ATE_RX_DEMOD
             || dtmModeType == RF_PHY_DTM_MODE_ATE_RX2TX
             || dtmModeType == RF_PHY_DTM_MODE_ATE_TX2RX)
   {
                        
		//===== rf cal		 
        rf_phy_ana_cfg();
        //rf_rxDcoc_cfg(/*rfChn*/88,/*bwSet*/1,&g_rfPhyRxDcIQ);		//set the rfChn as 2402 for BW=1MHz 
        rf_tpCal_cfg(rfChn);              
        ...
        PHY_REG_WT(0x40031024,dtmPktIntv+100);//timeout set 
        PHY_REG_WT(0x40031028,dtmPktIntv+100 );//timeout set 
        ...
        //set rx no timeout[31:16] + max packet len
        PHY_REG_WT(0x4003000c, (dtmLength+3)>255 ? 255: dtmLength+3 );
        
        //close ll hw irg
        ll_hw_set_irq(0);
        
        if(     dtmModeType == RF_PHY_DTM_MODE_ATE_RX2TX)
        {
            ll_hw_set_rtx();
            PHY_REG_WT(0x40031024,10000);//timeout set 
            PHY_REG_WT(0x40031028,10000);//timeout set

        }
        else if(dtmModeType == RF_PHY_DTM_MODE_ATE_TX2RX)
        {         
        ...
        //------------------------------------------------------------------------------
        // trigger and recoder the rx packet number
...
//        txPkt[1] = 38;
        ...
        if(     dtmModeType == RF_PHY_DTM_MODE_ATE_RX_AUTOGAIN 
            ||  dtmModeType == RF_PHY_DTM_MODE_ATE_RX_FIXGAIN )
        {
            
            PHY_REG_WT( 0x400300a0,0x0000000e);     // clr pll_auto override
            PHY_REG_WT( 0x400300a4,0x0000002a);     // rx_en rx_bb_en
            
            if(dtmModeType == RF_PHY_DTM_MODE_ATE_RX_AUTOGAIN){
                    
                PHY_REG_WT( 0x40030050,0x22085580);                     // auto gain
            }else{
                PHY_REG_WT( 0x40030050,0x22085590 | (0x0f&(cmdWord>>16)));      // fix gain,
            }                
            WaitUs(200);
            
            len     = 4;
            dOut[0] = 0xff & (PHY_REG_RD( 0x400300e8)>>24);     //rssi adc
            dOut[1] = 0xff & (PHY_REG_RD( 0x400300e8)>>16);     //rssi flt
            dOut[2] = 0xff & (PHY_REG_RD( 0x400300e8)>> 8);     //rx adc abs
            dOut[3] = 0x0f & (PHY_REG_RD( 0x400300e8));         //rx gain idx
        ...
        else
        {
            ...
            while(loopN)
            {
                ...
                if(ll_hw_get_irq_status()&LIRQ_MD)
                {
                     ...
                     if(dtmModeType == RF_PHY_DTM_MODE_ATE_TX2RX)
                     {
                         WaitUs(70);
                     }                     
