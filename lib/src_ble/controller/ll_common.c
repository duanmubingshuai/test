...
//    connId = connPtr->connId;
  ...
  */
                         ...
                         connPtr->chanMapUpdateEvent ) ) )
  {
    ...
    if(llEventDelta(connPtr->nextEvent,connPtr->chanMapUpdateEvent )>=0)
    {
...
{
//	#ifdef _PHY_DEBUG 
//		  LOG("%s,%s,Line %d,connPtr->allocConn %d\n",__FILE__,__func__,__LINE__,connPtr->allocConn);
//	#endif
	uint8_t  next;
...
//	#ifdef _PHY_DEBUG 
//				  LOG("next %d,llState %d,%d\n",next,llState,llSecondaryState );
//	#endif
...
//        LOG("header %x pktHdr %x,pktLen %d,connPtr->rxDataEnabled %d\n",connPtr->ll_buf.rx_conn_desc[i]->header,pktHdr,pktLen,connPtr->rxDataEnabled);
...
uint8 llSetupVersionIndReq( llConnState_t *connPtr )
{
    ...
    {		
        ...
        *pBuf++ = LL_VERSION_NUM;
		*pBuf++ = LO_UINT16(LL_COMPANY_ID);
		*pBuf++ = HI_UINT16(LL_COMPANY_ID);
//		uint16 comId = LL_COMPANY_ID;
//        // write the company ID
//        pBuf = llMemCopyDst( pBuf, (uint8 *)comId, 2 );

        // write the subversion number
//        uint8 subverNum = LL_SUBVERSION_NUM;
//        pBuf = llMemCopyDst( pBuf, (uint8 *)&subverNum, 2 );
		*pBuf++ = LO_UINT16(LL_SUBVERSION_NUM);
		*pBuf++ = HI_UINT16(LL_SUBVERSION_NUM);
...
void llResetConnId(  uint8 connId )
{
	...
	osal_memset(&conn_param[connId], 0,(sizeof(llConnState_t) - sizeof(llLinkBuf_t)));
	connPtr->connId = connId;
...
//    connPtr->llPduLen.isChanged	  =FALSE;
	...
	// add 2020-03-17
	connPtr->llRfPhyPktFmt  = g_rfPhyPktFmt;// LE_1M_PHY;
	// add 2020-04-21
...
{
    osal_memset(&rfCounters, 0, sizeof(rfCounters_t));
