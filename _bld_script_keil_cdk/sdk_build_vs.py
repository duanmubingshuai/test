...
# home_path = "C:\\Dfile\\Git\\pico_rom_code\\pico_sdk\\rls_v610\\"
home_path = "C:\\Dfile\\Git\\" or "D:\\Dfile\\Git\\" or "E:\\Dfile\\Git\\"
'''
...
def root_dirname(dirnames):
	for (root, dirs, files) in os.walk(home_path):
		# for file_name in files:
		for dir_name in dirs:
           # if "librf.a" in file_name:
			# if "rls_v610_commit" in dir_name:
			if dirnames in dir_name and dirnames+"_" not in dir_name:
				result = os.path.join(root,dir_name)#.replace('\\','/')
				# i += 1
				# j = [i]
				# print(j+result)
	return result
...
class build:
	...
	def __call__(self, build_param = None, output = None):
		...
		if(build_param is not None):
			...
			filename = re.split('[.]', self.m_path)
			dirnames = "rls_v610_commit"
			result = root_dirname(dirnames)
			
			file_n = result + filename[0] + filename[1] + filename[2] + '.cdkproj'
			fprj = open(file_n, 'wb')
