...
def commit_id():
	...
	c_flg = False
	proj_items = []
	file_n = '..\\lib\\generate_lib\\rf.uvprojx'
...
def commit_date():
	...
	fdata = '_date.txt'
	delete_date()
	# cmd = 'git log --pretty=format:"%ad" --date=local -1 ' + ' 1>>_date.txt 2>&1 '
	cmd = 'git log --pretty=format:"%ad" --date=format:"%Y%m%d%H%M%S" -1 ' + ' 1>>_date.txt 2>&1 '
	os.system(cmd)
	date = read_date(fdata)
	'''
	date = date_finder()
