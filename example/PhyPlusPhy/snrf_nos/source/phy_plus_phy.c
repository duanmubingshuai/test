...
uint8_t adv_buffer[PHYPLUS_DATA_MAX_LENGTH];
...
void phy_rx_data_process(void)
{
    ...
    if(osal_memcmp(&(phyBufRx[2]),s_pubAddr,6))
    {
        PRINT("-------------------------\n");
...
void PhyPlusPhy_Init(uint8 task_id)
{
    ...
    //init tx buf
    osal_memset(phyBufTx,0,PHYPLUS_RF_MAX_LENGTH);   
    ...
    //adv buffer init
    {
        ...
        adv_buffer[29] = 0x56;
        adv_buffer[30] = 0xE6;
        uint8_t advHead[2]= {0x00,(PHYPLUS_DATA_MAX_LENGTH+6)};
        //tx buf date update
        phy_tx_buf_updata(s_pubAddr,advHead,adv_buffer,PHYPLUS_DATA_MAX_LENGTH);
    }
    //phy pktfmt config
    ...
    s_pktCfg.pduLen     =   PHYPLUS_DATA_MAX_LENGTH+6;
