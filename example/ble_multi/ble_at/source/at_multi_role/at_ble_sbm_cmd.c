...
const CLI_COMMAND ble_at_cmd_list[30] =
{
  ///union cmd
  ...
  { "AT+CONNCANCEL", "cancel conn", at_conn_cancel},
#endif

  ///conn cmd
  //master & slave role support
  { "AT+DISCONN", "terminate conn", at_conn_terminate},
  { "AT+DLU", "set data length", at_dlu},
...
static uint16_t at_set_scan_parameters(uint32_t argc, unsigned char *argv[])
{
    uint16_t scan_interval,scan_window,scan_duration;
...
static uint16_t at_dlu(uint32_t argc, unsigned char *argv[])
{
    ...
    if(argv[0][0] == '=')
    {
      ...
      if( conn_handle >= GAP_CONNHANDLE_INIT|| tx_octets < 0x001B || tx_octets > 0x00FB )
      {
        AT_LOG("\r\n+ERROR=%d\r\n",ERROR_INVALID_PARAMETERS);
        return ERROR_INVALID_PARAMETERS;
      } 
...
static uint16_t at_conn_para_update(uint32_t argc, unsigned char *argv[])
{
    ...
    if(argv[0][0] == '=')
    {
      ...
      if( conn_handle >= GAP_CONNHANDLE_INIT|| at_parameters.conn_param[conn_handle].con_role_state != Slave_Role )
      {
        AT_LOG("\r\n+ERROR=%d\r\n",ERROR_INVALID_PARAMETERS);
        return ERROR_INVALID_PARAMETERS;
      } 
...
// conn_handle  att_handle  ntf_length  ntf_value
static uint16_t at_notify(uint32_t argc, unsigned char *argv[])
{
    uint16_t       conn_handle;
    if(argv[0][0] == '=')
    {
      ...
      if(at_parameters.conn_param[conn_handle].con_role_state == Slave_Role)
      {
        ...
        ntf.handle = CLI_strtoi(argv[2], CLI_strlen(argv[2]), 10);
        ntf.len = CLI_strtoi(argv[3], CLI_strlen(argv[3]), 10);
        uint8_t return_status = CLI_strtoarray(argv[4], CLI_strlen(argv[4]), ntf.value,ntf.len);
        if( return_status != OK_RETURN_SUCCESS_STATE )
        {
          AT_LOG("\r\n+ERROR=%d\r\n",ERROR_INVALID_PARAMETERS);
          return ERROR_INVALID_PARAMETERS;
...
static uint16_t at_mtu(uint32_t argc, unsigned char *argv[])
{
    ...
    if(argv[0][0] == '=')
    {
      ...
      if( conn_handle >= GAP_CONNHANDLE_INIT|| mtu_size < ATT_MTU_SIZE_MIN || mtu_size > ATT_MAX_MTU_SIZE )
      {
        AT_LOG("\r\n+ERROR=%d\r\n",ERROR_INVALID_PARAMETERS);
        return ERROR_INVALID_PARAMETERS;
      } 
...
__ATTR_SECTION_SRAM__ void at_wakeup_handle(void)
{
  ...
  osal_start_timerEx( bleAT_TaskID, AT_ENTER_ATUO_SLEEP_MODE_EVT, 5*1000 );
