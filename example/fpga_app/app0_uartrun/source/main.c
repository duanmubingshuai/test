...
void init_config(void)
{
    ...
    pGlobal_config[LL_SWITCH] =  SLAVE_LATENCY_ALLOW | RC32_TRACKINK_ALLOW
                                | LL_RC32K_SEL | SIMUL_CONN_SCAN_ALLOW ; //
    ...
    pGlobal_config[CLK_TRACKING_CONFIG_DLL ] = 0;//0x000a01f4;//[23:16] settle_thd [11:0] target_cnt
    pGlobal_config[CLK_TRACKING_CONFIG_XTAL] = 0;//0x000a00fa;//[23:16] settle_thd [11:0] target_cnt
...
{     
    ...
    g_system_clk = SYS_CLK_RC_32M;//SYS_CLK_DLL_32M //SYS_CLK_XTAL_16M //SYS_CLK_DLL_48M
...
//	LOG("otp bridge %x\n",read_reg(0x40000028));
	...
	gpio_dir(GPIO_P11,GPIO_OUTPUT);
	gpio_write(GPIO_P11,1);
	gpio_write(GPIO_P11,0);
	gpio_write(GPIO_P11,1);
	drv_disable_irq();
 	//cm_main();
	gpio_dir(GPIO_P11,GPIO_OUTPUT);
...
{     
	...
	two_stage_app_uartrun(__Vectors);
