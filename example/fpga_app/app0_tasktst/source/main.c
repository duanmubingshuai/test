...
//	hal_gpio_toggle(GPIO_P11,5);
//	uint32 vari;
//	for(uint32 tiktok=0;tiktok <100;tiktok ++){
//		vari = vari + tiktok;
//	}
//	uint32_t v26Addr;
...
//	uint32_t v27Addr;
//	JUMP_FUNCTION_SET(V27_IRQ_HANDLER, (uint32_t)&debug_inter);
//	v27Addr = JUMP_FUNCTION_GET(V27_IRQ_HANDLER);
	while(1)
	{
		;
	}
...
{     
    ...
    JUMP_FUNCTION_SET(CK802_TRAP_C, (uint32_t)trap_app);
    
    bx_to_application((uint32_t)&trap_entry);
#endif /*APP_TRAP_TEST*/
