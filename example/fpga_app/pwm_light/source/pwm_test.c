...
void pwm_test1(void)
{
	...
	const uint8 pwmPin[] = {P0,P1,P2,P3,P6,P7,P8,P11,P12,P13,P14,P15,P18,P19};	
	LOG("pwm_test_in\n");
	//basic test
	while(0)
	{
		pwm0.pwm_start(&pwm0);
		ret = pwm0.pwm_state(&pwm0);
		LOG("ret:%d\n",ret);
		WaitMs(1000);
	...
	switch(testcase)
	{
		case 1://iomux for pwm 
			...
			for(i=0;i<sizeof(pwmPin)/sizeof(pwmPin[0]);i++)
			{
				if((pwmPin[i] == P2) || (pwmPin[i] == P3))
				{
					continue;
				}				
				pwm0.pwmPin = pwmPin[i];
