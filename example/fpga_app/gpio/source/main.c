...
void hal_init(void)
{
    ...
    LOG("---------->");
	LOG("1:0x%x\n",hal_gpio_IRQ);
	LOG("2:0x%x\n",((*(uint16 *)(JUMP_BASE_ADDR + (jkk << 1))) + (SRAM_BASE_ADDR&0xffff0000)));
...
//    pGlobal_config[CLK_TRACKING_CONFIG_DLL ] = 0x000a01f4;//0;//[23:16] settle_thd [11:0] target_cnt
//    pGlobal_config[CLK_TRACKING_CONFIG_XTAL] = 0x000a00fa;//0;//[23:16] settle_thd [11:0] target_cnt
	...
	pGlobal_config[MIN_SLEEP_TIME] = 10000;  
...
{     
    ...
    g_system_clk = SYS_CLK_DLL_48M; //SYS_CLK_XTAL_16M;//SYS_CLK_RC_32M;//SYS_CLK_DLL_32M; //SYS_CLK_DLL_64M; //
...
//	pwrmgr_config(PWR_MODE_SLEEP);
	hal_pwrmgr_init();
//	pwrmgr_init();
//	pwrmgr_clk_gate_config(MOD_USR0);
    pwrmgr_register(MOD_USR0, test, rf_wakeup_handler);
