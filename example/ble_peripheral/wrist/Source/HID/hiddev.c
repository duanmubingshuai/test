...
void ll_dumpConnectionInfo(llConnState_t* connPtr, uint8 reason )
{
    if (connPtr != NULL)
    {
        LOG("ll terminate reason = %d\r\n", reason);
        LOG("========== Connection information ================\r\n");
        LOG("currentEvent = %d, nextEvent = %d, expirationEvent = %d\r\n", connPtr->currentEvent, connPtr->nextEvent, connPtr->expirationEvent);
        LOG("slaveLatencyAllowed = %d, slaveLatency = %d\r\n", connPtr->slaveLatencyAllowed, connPtr->slaveLatencyValue);
        LOG("current connInterval = %d\r\n", connPtr->curParam.connInterval);
        LOG("current connTimeout = %d\r\n", connPtr->curParam.connTimeout);
        LOG("paramUpdateEvent = %d\r\n", connPtr->paramUpdateEvent);
        LOG("connPtr->paramUpdate.connInterval = %d\r\n", connPtr->paramUpdate.connInterval);
    }

    LOG("========== LL PM counters ================\r\n");
    LOG("ll_send_undirect_adv_cnt = %d\r\n", g_pmCounters.ll_send_undirect_adv_cnt);
    LOG("ll_send_nonconn_adv_cnt = %d\r\n", g_pmCounters.ll_send_nonconn_adv_cnt);
    LOG("ll_recv_scan_req_cnt = %d\r\n", g_pmCounters.ll_recv_scan_req_cnt);
    LOG("ll_send_scan_rsp_cnt = %d\r\n", g_pmCounters.ll_send_scan_rsp_cnt);
    LOG("ll_recv_conn_req_cnt = %d\r\n", g_pmCounters.ll_recv_conn_req_cnt);
    LOG("ll_filter_scan_req_cnt = %d\r\n", g_pmCounters.ll_filter_scan_req_cnt);
    LOG("ll_filter_conn_req_cnt = %d\r\n", g_pmCounters.ll_filter_conn_req_cnt);
//    LOG("ll_recv_adv_pkt_cnt = %d\r\n", g_pmCounters.ll_recv_adv_pkt_cnt);
//    LOG("ll_recv_scan_rsp_cnt = %d\r\n", g_pmCounters.ll_recv_scan_rsp_cnt);
    LOG("ll_conn_succ_cnt = %d\r\n", g_pmCounters.ll_conn_succ_cnt);
    LOG("ll_link_lost_cnt = %d\r\n", g_pmCounters.ll_link_lost_cnt);
    LOG("ll_link_estab_fail_cnt = %d\r\n", g_pmCounters.ll_link_estab_fail_cnt);
//    LOG("ll_recv_ctrl_pkt_cnt = %d\r\n", g_pmCounters.ll_recv_ctrl_pkt_cnt);
//    LOG("ll_recv_data_pkt_cnt = %d\r\n", g_pmCounters.ll_recv_data_pkt_cnt);
//    LOG("ll_recv_invalid_pkt_cnt = %d\r\n", g_pmCounters.ll_recv_invalid_pkt_cnt);
//
//    LOG("ll_recv_abnormal_cnt = %d\r\n", g_pmCounters.ll_recv_abnormal_cnt);
//    LOG("ll_send_data_pkt_cnt = %d\r\n", g_pmCounters.ll_send_data_pkt_cnt);
//    LOG("ll_conn_event_cnt = %d\r\n", g_pmCounters.ll_conn_event_cnt);
//    LOG("ll_recv_crcerr_event_cnt = %d\r\n", g_pmCounters.ll_recv_crcerr_event_cnt);
//    LOG("ll_conn_event_timeout_cnt = %d\r\n", g_pmCounters.ll_conn_event_timeout_cnt);
//
...
static void hidDevPairStateCB( uint16 connHandle, uint8 state, uint8 status )
{
    ...
    else if ( state == GAPBOND_PAIRING_STATE_COMPLETE )
    {
        ...
        if ( status == SUCCESS )
        {
            ...
            osal_start_timerEx(hidDevTaskId, HID_UPPARAM_EVT, 2000);
            osal_start_timerEx(AncsApp_TaskID, ANCSAPP_START_DISC_EVT, 5000);
        }
        else
        {
            LOG("Pair Fail\n\r");
    ...
    else if ( state == GAPBOND_PAIRING_STATE_BONDED )
    {
        if ( status == SUCCESS )
        {
            ...
            // osal_start_timerEx(hidKbdTaskId, HID_TEST_EVT, 5000);
            osal_start_timerEx(hidDevTaskId, HID_UPPARAM_EVT, 2000);
            osal_start_timerEx(AncsApp_TaskID, ANCSAPP_START_DISC_EVT, 8000);
            LOG("bond Success\n\r");
        }
    }
