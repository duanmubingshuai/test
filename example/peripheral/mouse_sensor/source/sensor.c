...
uint8 sensorReadByte(uint8 addr)
{
  ...
  gpio_pull_set((gpio_pin_e)(s_spi_cfg.sclk_pin.pin), GPIO_PULL_UP);
  gpio_fmux_set((gpio_pin_e)(s_spi_cfg.sclk_pin.pin), 0);
  gpio_write((gpio_pin_e)(s_spi_cfg.sclk_pin.pin),1);
  gpio_dir((gpio_pin_e)(s_spi_cfg.sclk_pin.pin),GPIO_OUTPUT);
  return rxbuf[0];
}
...
uint16 msensor_process_event(uint8 task_id, uint16 events )
{
      ...
      if ( events & MOUSE_MOTION_STATUS_CHECK_LOOP_EVT )
      {
          ...
          if(1){   //(hidDevGapState==GAPROLE_CONNECTED) && HIDMouse_bond_ready) {            
              if(Motion_status == (0x80|(HidMouseSensor.cur_DPI)))
              {
                  ...
                  //hidKbdSendMouseReport(0,(uint8)d_x,(uint8)d_y,0);
              }
              else {
  ...
  //                      }
  //                    } 
                      osal_start_timerEx(msensor_TaskID, MOUSE_MOTION_STATUS_CHECK_LOOP_EVT, 20);//200);            
                  }
              }
          }
          else
