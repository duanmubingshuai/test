...
KSCAN_ROWS_e rows[NUM_KEY_ROWS] = {KEY_ROW_P00,KEY_ROW_P02,KEY_ROW_P23,KEY_ROW_P18};
KSCAN_COLS_e cols[NUM_KEY_COLS] = {KEY_COL_P01,KEY_COL_P03,KEY_COL_P24,KEY_COL_P11};
...
void KSCAN_Init( uint8 task_id )
{
    ...
    osal_start_reload_timer(KSCAN_TaskID,KSCAN_TIMER_RELOAD_EVT,1000);
}
...
uint16 KSCAN_ProcessEvent( uint8 task_id, uint16 events )
{
    ...
    if ( events & KSCAN_TIMER_RELOAD_EVT )
    {
        LOG("run ");
        return (events ^ KSCAN_TIMER_RELOAD_EVT);
    }
