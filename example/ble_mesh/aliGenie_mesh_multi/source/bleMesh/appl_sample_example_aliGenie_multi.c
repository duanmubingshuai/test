...
//                proxy_dly_thandle1 = EM_TIMER_HANDLE_INIT_VAL;
//                proxy_dly_thandle = EM_TIMER_HANDLE_INIT_VAL;
//                hal_gpio_write(P3,1);
//                CONSOLE_OUT("[state] delay\n");
//                EM_RESULT ret;
//                ret=EM_start_timer (&proxy_dly_thandle1,1, NULL, NULL, 0);
//                ret=EM_start_timer (&proxy_dly_thandle,(EM_TIMEOUT_MILLISEC | 350), proxy_dly_generic_hsl, NULL, 0);
//                CONSOLE_OUT("osal_sys_tick_1:%d,ret=%d\n",osal_sys_tick,ret);
        }
        else
        {
            UI_light_hsl_set_actual(0,UI_light_hsl.hsl_lightness,UI_light_hsl.hsl_hue,UI_light_hsl.hsl_saturation,0);
...
void UI_vendor_dump_bytes(UCHAR* buffer, UINT16 length)
{
    UINT16 byte;
...
/*********************************************************************
    LOCAL FUNCTIONS
*/
API_RESULT UI_vendormodel_send_reliable_pdu(
...
)
{
    ...
    {
        CONSOLE_OUT(
            "MS_ACCESS_PHYPLUSMODEL_WRITECMD_OPCODE\n");
    }
    break;
...
/** Provisioning Handle */
DECL_STATIC PROV_HANDLE UI_prov_brr_handle;
...
API_RESULT UI_set_brr_scan_rsp_data (void)
{
    ...
    */
    ...
    CONSOLE_OUT("\n Setting ALI PHY MSH Device as Complete Device Name!\n");
...
//    MS_DISABLE_FRIEND_FEATURE();
    ...
    MS_access_cm_set_transmit_state(MS_NETWORK_TX_STATE, (1<<3)|0);
    #ifndef BLEBRR_LP_SUPPORT

    /* Fetch PROXY feature state */
    if(UI_prov_brr_handle == PROV_BRR_ADV)
    {
        MS_access_cm_get_features_field(&proxy_state, MS_FEATURE_PROXY);

        if (MS_ENABLE == proxy_state)
        {
